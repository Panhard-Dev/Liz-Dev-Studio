import { createEffect, createMemo, createSignal, type ComponentProps, splitProps, Show } from "solid-js"
...
export function Avatar(props: AvatarProps) {
  ...
  const src = createMemo(() => (split.src && !imageFailed() ? split.src : undefined))
  ...
  return (
    ...
    >
      <Show when={src()} fallback={first(split.fallback)}>
        {(src) => <img src={src()} draggable={false} data-slot="avatar-image" onError={() => setImageFailed(true)} />}
      </Show>
    </div>
  )
}
