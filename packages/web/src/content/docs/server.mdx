---
title: Server
description: Interact with liz server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `liz serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an liz client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `liz.local` |
...
Set `LIZ_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `liz`, or set `LIZ_SERVER_USERNAME` to override it. This applies to both `liz serve` and `liz web`.
...
When you run `liz` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the liz server to interact with liz programmatically.
:::

This architecture lets liz support multiple clients and allows you to interact with liz programmatically.

You can run `liz serve` to start a standalone server. If you have the
liz TUI running, `liz serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the LIZ AI BRASIL [IDE](/docs/ide) plugins.
...
The liz server exposes the following APIs.
