---
title: SDK
description: Client JS de type sécurisé pour le serveur liz.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK liz JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler liz par programme.
...
Créez une instance de liz :

```javascript
import { createLiz } from "@liz-ai-brasil/sdk"

const { client } = await createLiz()
```
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `liz.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :
...
console.log(`Server running at ${liz.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de liz, vous pouvez créer une instance client pour vous y connecter :
...
```typescript
import type { Session, Message, Part } from "@liz-ai-brasil/sdk"
```
