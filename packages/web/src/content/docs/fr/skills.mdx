...
Les compétences d'agent permettent à LIZ AI BRASIL de découvrir des instructions réutilisables à partir de votre dépôt ou de votre répertoire personnel.
...
LIZ AI BRASIL recherche ces emplacements :

- Configuration du projet : `.liz/skills/<name>/SKILL.md`
- Configuration globale : `~/.config/liz/skills/<name>/SKILL.md`
...
Pour les chemins locaux du projet, LIZ AI BRASIL remonte de votre répertoire de travail actuel jusqu'à ce qu'il atteigne l'arbre de travail git.
Il charge tout `skills/*/SKILL.md` correspondant dans `.liz/` et tout `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` correspondant en cours de route.

Les définitions globales sont également chargées à partir de `~/.config/liz/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` et `~/.agents/skills/*/SKILL.md`.
...
Créez `.liz/skills/git-release/SKILL.md` comme ceci :
...
LIZ AI BRASIL répertorie les compétences disponibles dans la description de l'outil `skill`.
...
Contrôlez les compétences auxquelles les agents peuvent accéder à l'aide d'autorisations basées sur des modèles dans `liz.json` :
...
**Pour les agents intégrés** (dans `liz.json`) :
