...
Avec LIZ AI BRASIL, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, LIZ AI BRASIL utilise notre propre thème `liz`.
...
LIZ AI BRASIL est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que LIZ AI BRASIL corresponde à l'apparence de leur terminal
...
LIZ AI BRASIL prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/liz/themes/*.json` ou `$XDG_CONFIG_HOME/liz/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.liz/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.liz/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/liz/themes
vim ~/.config/liz/themes/my-theme.json
```
...
{
  "$schema": "https://liz.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
