...
Il existe deux types d'agents dans LIZ AI BRASIL ; agents primaires et sous-agents.
...
LIZ AI BRASIL est livré avec deux agents primaires intégrés, **Build** et **Plan**.
...
LIZ AI BRASIL est livré avec deux sous-agents intégrés, **General** et **Explore**. Nous verrons cela ci-dessous.

---

## Agents intégrés

LIZ AI BRASIL est livré avec deux agents primaires intégrés et deux sous-agents intégrés.
...
Configurez les agents dans votre fichier de configuration `liz.json` :
...
- Global : `~/.config/liz/agents/`
- Par projet : `.liz/agents/`

```markdown title="~/.config/liz/agents/review.md"
---
...
Si aucune température n'est spécifiée, LIZ AI BRASIL utilise les valeurs par défaut spécifiques au modèle ; généralement 0 pour la plupart des modèles, 0,55 pour les modèles Qwen.
...
Ce chemin est relatif à l'emplacement du fichier de configuration. Cela fonctionne donc à la fois pour la configuration globale LIZ AI BRASIL et pour la configuration spécifique au projet.
...
L'ID de modèle dans votre configuration LIZ AI BRASIL utilise le format `provider/model-id`. Par exemple, si vous utilisez [LIZ AI BRASIL Zen](/docs/zen), vous utiliserez `liz/gpt-5.1-codex` pour GPT 5.1 Codex.
...
:::tip
Exécutez `liz models` pour voir une liste des modèles disponibles.
:::
...
:::tip
Avez-vous un agent que vous aimeriez partager ? [Soumettez un PR](https://github.com/anomalyco/liz).
:::
...
```markdown title="~/.config/liz/agents/docs-writer.md"
---
...
```markdown title="~/.config/liz/agents/security-auditor.md"
---
