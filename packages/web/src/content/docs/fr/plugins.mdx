---
title: Plugins
description: Écrivez vos propres plugins pour étendre LIZ AI BRASIL.
---

Les plugins vous permettent d'étendre LIZ AI BRASIL en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de LIZ AI BRASIL.
...
- `.liz/plugins/` - Plugins au niveau du projet
- `~/.config/liz/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/liz/node_modules/`.
...
1. Configuration globale (`~/.config/liz/liz.json`)
2. Configuration du projet (`liz.json`)
3. Répertoire global des plugins (`~/.config/liz/plugins/`)
4. Répertoire des plugins du projet (`.liz/plugins/`)
...
LIZ AI BRASIL exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK liz pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre liz.
...
:::note
Si vous utilisez l'application de bureau LIZ AI BRASIL, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez liz de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à liz :
...
L'assistant `tool` crée un outil personnalisé que liz peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour liz aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://liz.ai/docs/sdk) pour plus de détails.
