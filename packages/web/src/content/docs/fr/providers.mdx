---
title: Fournisseurs
description: Utiliser n’importe quel fournisseur LLM en LIZ AI BRASIL.
---

import config from "../../../../config.mjs"
export const console = config.console

LIZ AI BRASIL utilise [AI SDK](https://ai-sdk.dev/) et [Models.dev](https://models.dev) pour prendre en charge **75+ fournisseurs LLM** et prend en charge l'exécution de modèles locaux.

Pour ajouter un fournisseur, vous devez :

1. Ajoutez les clés API pour le fournisseur à l'aide de la commande `/connect`.
2. Configurez le fournisseur dans votre configuration LIZ AI BRASIL.
...
en `~/.local/share/liz/auth.json`.

---

### Configuration

Vous pouvez personnaliser les fournisseurs via la section `provider` de votre LIZ AI BRASIL
configuration.
...
LIZ AI BRASIL Zen est une liste de modèles fournis par l'équipe LIZ AI BRASIL qui ont été
testé et vérifié pour fonctionner correctement avec LIZ AI BRASIL. [En savoir plus](/docs/zen).

:::tip
Si vous êtes nouveau, nous vous recommandons de commencer par LIZ AI BRASIL Zen.
:::

1. Exécutez la commande `/connect` dans le TUI, sélectionnez liz et dirigez-vous vers [liz.ai/auth](https://liz.ai/auth).
...
Il fonctionne comme n’importe quel autre fournisseur dans LIZ AI BRASIL et son utilisation est totalement facultative.

---

## LIZ AI BRASIL Go

LIZ AI BRASIL Go est un plan d'abonnement à faible coût qui offre un accès fiable aux modèles de codage ouverts populaires fournis par l'équipe LIZ AI BRASIL qui ont été
testé et vérifié pour fonctionner correctement avec LIZ AI BRASIL.

1. Exécutez la commande `/connect` dans le TUI, sélectionnez `LIZ AI BRASIL Go` et rendez-vous sur [liz.ai/auth](https://liz.ai/zen).
...
Pour utiliser Amazon Bedrock avec LIZ AI BRASIL :
...
Définissez l'une de ces variables d'environnement lors de l'exécution de liz :
...
Pour une configuration spécifique au projet ou persistante, utilisez `liz.json` :
...
:::info
L'utilisation de votre abonnement Claude Pro/Max dans LIZ AI BRASIL n'est pas officiellement prise en charge par [Anthropic](https://anthropic.com).
:::
...
Vous pouvez configurer liz pour utiliser des modèles locaux via [Atomic Chat](https://atomic.chat), une application de bureau qui exécute des LLM locaux derrière un serveur API compatible OpenAI (point de terminaison par défaut `http://127.0.0.1:1337/v1`).
...
2. Accédez à [Azure AI Foundry](https://ai.azure.com/) et déployez un modèle.
   ...
   :::note
   Le nom du déploiement doit correspondre au nom du modèle pour que liz fonctionne correctement.
   :::
...
Vous pouvez également ajouter des modèles via votre configuration liz.
...
3.  Nom : `LIZ AI BRASIL`, Portées : `api`
...
pour stocker le jeton dans le stockage d'authentification liz.
:::

##### GitLab auto-hébergé

:::note[note de conformité]
LIZ AI BRASIL utilise un petit modèle pour certaines tâches d'IA telles que la génération du titre de la session.
Il est configuré pour utiliser gpt-5-nano par défaut, hébergé par Zen. Pour verrouiller LIZ AI BRASIL
pour utiliser uniquement votre propre instance hébergée par GitLab, ajoutez ce qui suit à votre
Fichier `liz.json`. Il est également recommandé de désactiver le partage de session.
...
Plus de documentation sur la page d'accueil [liz-gitlab-auth](https://www.npmjs.com/package/liz-gitlab-auth).

##### Configuration

Personnalisez via `liz.json` :
...
Pour utiliser votre abonnement GitHub Copilot avec liz :
...
Pour utiliser Google Vertex AI avec LIZ AI BRASIL :
...
Définissez-les lors de l'exécution de liz.
...
Si vous voyez une fonctionnalité ou un modèle d'Helicone qui n'est pas configuré automatiquement via liz, vous pouvez toujours le configurer vous-même.
...
La fonctionnalité [Sessions](https://docs.helicone.ai/features/sessions) d'Helicone vous permet de regrouper les requêtes LLM associées. Utilisez le plugin [liz-helicone-session](https://github.com/H2Shami/liz-helicone-session) pour enregistrer automatiquement chaque conversation LIZ AI BRASIL en tant que session dans Helicone.
...
Le plugin injecte les en-têtes `Helicone-Session-Id` et `Helicone-Session-Name` dans vos requêtes. Sur la page Sessions d'Helicone, vous verrez chaque conversation LIZ AI BRASIL répertoriée comme une session distincte.
...
Vous pouvez configurer liz pour utiliser des modèles locaux via l'utilitaire llama-server de [llama.cpp's](https://github.com/ggml-org/llama.cpp)
...
Vous pouvez configurer liz pour utiliser des modèles locaux via LM Studio.
...
Vous pouvez configurer liz pour utiliser des modèles locaux via Ollama.

:::tip
Ollama peut se configurer automatiquement pour LIZ AI BRASIL. Voir les [documents d'intégration Ollama](https://docs.ollama.com/integrations/liz) pour plus de détails.
:::
...
Pour utiliser Ollama Cloud avec LIZ AI BRASIL :
...
3. Copiez la clé API à utiliser dans LIZ AI BRASIL.
...
6. **Important** : Avant d'utiliser des modèles cloud dans LIZ AI BRASIL, vous devez extraire les informations du modèle localement :
...
LIZ AI BRASIL Zen est une liste de modèles testés et vérifiés fournie par l'équipe LIZ AI BRASIL. [En savoir plus](/docs/zen).

1. Connectez-vous à **<a href={console}>LIZ AI BRASIL Zen</a>** et cliquez sur **Créer une clé API**.

2. Exécutez la commande `/connect` et recherchez **LIZ AI BRASIL Zen**.

   ```txt
   /connect
   ```

3. Entrez votre clé LIZ AI BRASIL API.
...
Vous pouvez également ajouter des modèles supplémentaires via votre configuration liz.
...
5. Vous pouvez également les personnaliser via votre configuration liz. Voici un exemple de spécification d'un fournisseur
...
Pour utiliser [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) avec Liz :
...
Vous pouvez également personnaliser les modèles via votre configuration liz. Voici un exemple de spécification de l'ordre de routage du fournisseur.
...
:::tip
Vous pouvez utiliser n'importe quel fournisseur compatible OpenAI avec liz. La plupart des fournisseurs d'IA modernes proposent des API compatibles OpenAI.
:::
...
4. Créez ou mettez à jour votre fichier `liz.json` dans le répertoire de votre projet :
...
Les champs `limit` permettent à LIZ AI BRASIL de comprendre la quantité de contexte qu'il vous reste. Les fournisseurs standard les extraient automatiquement de models.dev.
...
1. **Vérifiez la configuration de l'authentification** : exécutez `liz auth list` pour voir si les informations d'identification
...
2. Pour les fournisseurs personnalisés, vérifiez la configuration liz et :
   - Assurez-vous que l'ID du fournisseur utilisé dans la commande `/connect` correspond à l'ID de votre configuration liz.
