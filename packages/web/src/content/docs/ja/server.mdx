---
title: サーバー
description: HTTP 経由で LIZ AI BRASIL サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`liz serve` コマンドは、LIZ AI BRASIL クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `liz.local` |
...
HTTP 基本認証でサーバーを保護するには、`LIZ_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `liz` になるか、`LIZ_SERVER_USERNAME` を設定してオーバーライドします。これは、`liz serve` と `liz web` の両方に当てはまります。
...
`liz` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
LIZ AI BRASIL サーバーを使用して、プログラムで LIZ AI BRASIL と対話します。
:::

`liz serve` を実行してスタンドアロンサーバーを起動できます。すでに LIZ AI BRASIL TUI を実行している場合でも、`liz serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、LIZ AI BRASIL [IDE](/docs/ide) プラグインによって使用されます。
...
LIZ AI BRASIL サーバーは次の API を公開します。
