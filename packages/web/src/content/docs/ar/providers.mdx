---
title: المزوّدون
description: استخدام أي موفّر LLM في LIZ AI BRASIL.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم LIZ AI BRASIL ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات LIZ AI BRASIL.
...
في `~/.local/share/liz/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات LIZ AI BRASIL.
...
LIZ AI BRASIL Zen هي قائمة نماذج يوفّرها فريق LIZ AI BRASIL وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع LIZ AI BRASIL. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع LIZ AI BRASIL Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر liz، ثم انتقل إلى [liz.ai/auth](https://liz.ai/auth).
...
يعمل مثل أي مزوّد آخر في LIZ AI BRASIL واستخدامه اختياري بالكامل.

---

## LIZ AI BRASIL Go

LIZ AI BRASIL Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق LIZ AI BRASIL، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع LIZ AI BRASIL.

1. شغّل الأمر `/connect` في TUI، واختر `LIZ AI BRASIL Go`، ثم انتقل إلى [liz.ai/auth](https://liz.ai/zen).
...
لاستخدام Amazon Bedrock مع LIZ AI BRASIL:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل liz:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `liz.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل LIZ AI BRASIL غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين liz لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل liz بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات liz.
...
تخزين الرمز في مساحة تخزين مصادقة liz.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم LIZ AI BRASIL نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر LIZ AI BRASIL
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`liz.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [liz-gitlab-auth](https://www.npmjs.com/package/liz-gitlab-auth).
...
لاستخدام اشتراك GitHub Copilot مع liz:
...
لاستخدام Google Vertex AI مع LIZ AI BRASIL:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر liz، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [liz-helicone-session](https://github.com/H2Shami/liz-helicone-session) لتسجيل كل محادثة LIZ AI BRASIL تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة LIZ AI BRASIL مدرجة كجلسة مستقلة.
...
يمكنك ضبط liz لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط liz لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط liz لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع LIZ AI BRASIL. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/liz) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع LIZ AI BRASIL:
...
3. انسخ مفتاح API لاستخدامه في LIZ AI BRASIL.
...
6. **مهم**: قبل استخدام نماذج السحابة في LIZ AI BRASIL، يجب سحب معلومات النموذج محليا:
...
LIZ AI BRASIL Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق LIZ AI BRASIL. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>LIZ AI BRASIL Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **LIZ AI BRASIL Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ LIZ AI BRASIL.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات liz.
...
5. يمكنك أيضا تخصيصها عبر إعدادات liz. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع Liz:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات liz. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع liz. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `liz.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ LIZ AI BRASIL بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `liz auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات liz وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات liz.
