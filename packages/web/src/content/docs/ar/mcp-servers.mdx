...
يمكنك إضافة أدوات خارجية إلى LIZ AI BRASIL باستخدام _Model Context Protocol_، أو MCP. يدعم LIZ AI BRASIL خوادم محلية وبعيدة.
...
يمكنك تعريف خوادم MCP في [إعدادات LIZ AI BRASIL](https://liz.ai/docs/config/) ضمن `mcp`. أضف كل خادم MCP باسم فريد. ويمكنك الإشارة إلى ذلك الخادم بالاسم عند توجيه الطلب إلى الـ LLM.
...
يمكن للمنظمات توفير خوادم MCP الافتراضية عبر نقطة النهاية `.well-known/liz` الخاصة بها. قد تكون هذه الخوادم معطّلة افتراضيا، مما يتيح للمستخدمين تفعيل ما يحتاجونه فقط.
...
يتولى LIZ AI BRASIL تلقائيا معالجة مصادقة OAuth لخوادم MCP البعيدة. عندما يتطلب خادم ما المصادقة، سيقوم LIZ AI BRASIL بما يلي:
...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك LIZ AI BRASIL المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `liz mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن LIZ AI BRASIL الرموز بشكل آمن في `~/.local/share/liz/mcp-auth.json`.
...
تتوفر خوادم MCP لديك كأدوات داخل LIZ AI BRASIL إلى جانب الأدوات المضمنة. لذا يمكنك إدارتها عبر إعدادات LIZ AI BRASIL مثل أي أداة أخرى.
...
سيؤدي ذلك إلى فتح نافذة متصفح لإكمال تدفق OAuth وربط LIZ AI BRASIL بحساب Sentry الخاص بك.
