---
title: الإضافات
description: اكتب إضافاتك الخاصة لتوسيع LIZ AI BRASIL.
---

تتيح لك الإضافات توسيع LIZ AI BRASIL عبر ربطها بأحداث مختلفة وتخصيص السلوك. يمكنك إنشاء إضافات لإضافة ميزات جديدة، أو التكامل مع خدمات خارجية، أو تعديل السلوك الافتراضي لـ LIZ AI BRASIL.
...
- `.liz/plugins/` - إضافات على مستوى المشروع
- `~/.config/liz/plugins/` - إضافات عامة
...
**إضافات npm** تُثبَّت تلقائيا باستخدام Bun عند بدء التشغيل. تُخزَّن الحزم واعتمادياتها مؤقتا في `~/.cache/liz/node_modules/`.
...
1. إعدادات عامة (`~/.config/liz/liz.json`)
2. إعدادات المشروع (`liz.json`)
3. دليل الإضافات العام (`~/.config/liz/plugins/`)
4. دليل إضافات المشروع (`.liz/plugins/`)
...
يشغّل LIZ AI BRASIL الأمر `bun install` عند بدء التشغيل لتثبيتها. بعد ذلك يمكن لإضافاتك وأدواتك استيرادها.
...
- `client`: عميل SDK لـ liz للتفاعل مع الذكاء الاصطناعي.
...
فيما يلي بعض أمثلة الإضافات التي يمكنك استخدامها لتوسيع liz.
...
:::note
إذا كنت تستخدم تطبيق LIZ AI BRASIL لسطح المكتب، فيمكنه إرسال إشعارات النظام تلقائيا عند جاهزية الرد أو عند حدوث خطأ في جلسة.
:::
...
يمكن للإضافات أيضا إضافة أدوات مخصصة إلى liz:
...
تقوم الدالة المساعدة `tool` بإنشاء أداة مخصصة يمكن لـ liz استدعاؤها. تأخذ دالة مخطط Zod وتعيد تعريف أداة يتضمن:

- `description`: ما الذي تفعله الأداة
- `args`: مخطط Zod لوسائط الأداة
- `execute`: الدالة التي تُشغَّل عند استدعاء الأداة

ستكون أدواتك المخصصة متاحة لـ liz إلى جانب الأدوات المضمنة.
...
المستويات: `debug`, `info`, `warn`, `error`. راجع [توثيق SDK](https://liz.ai/docs/sdk) للتفاصيل.
