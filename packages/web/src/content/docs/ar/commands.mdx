...
أنشئ الملف `.liz/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات LIZ AI BRASIL أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ LIZ AI BRASIL:
...
- على مستوى النظام: `~/.config/liz/commands/`
- لكل مشروع: `.liz/commands/`
...
يتضمن liz عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
