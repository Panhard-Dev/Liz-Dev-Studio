---
title: الخادم
description: تفاعل مع خادم liz عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `liz serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل liz استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `liz.local` |
...
عيّن `LIZ_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `liz`، أو عيّن `LIZ_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `liz serve` و `liz web`.
...
عند تشغيل `liz` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم liz للتفاعل مع liz برمجيا.
:::

تتيح هذه البنية لـ liz دعم عدة عملاء وتمكّنك من التفاعل مع liz برمجيا.

يمكنك تشغيل `liz serve` لبدء خادم مستقل. إذا كانت واجهة liz في terminal (TUI)
قيد التشغيل، فسيبدأ `liz serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات LIZ AI BRASIL لـ [IDE](/docs/ide).
...
يعرض خادم liz واجهات API التالية.
