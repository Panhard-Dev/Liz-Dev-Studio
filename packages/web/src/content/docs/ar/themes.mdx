...
مع LIZ AI BRASIL يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم LIZ AI BRASIL سمتنا `liz`.
...
يأتي LIZ AI BRASIL مع عدة سمات مدمجة.
...
- يريدون أن يطابق LIZ AI BRASIL مظهر terminal لديهم
...
يدعم LIZ AI BRASIL نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/liz/themes/*.json` أو `$XDG_CONFIG_HOME/liz/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.liz/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.liz/themes/*.json`
