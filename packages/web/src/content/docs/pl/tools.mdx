...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. LIZ AI BRASIL posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w LIZ AI BRASIL.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `LIZ_EXPERIMENTAL_LSP_TOOL=true` (lub `LIZ_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy LIZ AI BRASIL lub gdy zmienna środowiskowa `LIZ_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu LIZ AI BRASIL:
