...
Umiejętności agenta pozwalają liz odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
liz przeszukuje te lokalizacje:

- Project config: `.liz/skills/<name>/SKILL.md`
- Global config: `~/.config/liz/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu liz przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.liz/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/liz/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.liz/skills/git-release/SKILL.md` w ten sposób:
...
liz wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `liz.json`:
...
**Dla agentów wbudowanych** (w `liz.json`):
