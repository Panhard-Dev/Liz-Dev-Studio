---
title: Serwer
description: Interakcja z serwerem liz za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `liz serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient liz.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `liz.local` |
...
Ustaw `LIZ_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `liz` lub ustaw `LIZ_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `liz serve`, jak i `liz web`.
...
Kiedy uruchomisz `liz`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera liz do programowej interakcji z liz.
:::

Ta architektura umożliwia obsługę wielu klientów przez liz i programową interakcję z kodem otwartym.

Możesz uruchomić `liz serve`, aby uruchomić samodzielny serwer. Jeśli masz
liz TUI działa, `liz serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki liz [IDE](/docs/ide).
...
Serwer liz udostępnia następujące interfejsy API.
