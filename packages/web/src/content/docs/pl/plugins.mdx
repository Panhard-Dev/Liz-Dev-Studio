---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć liz.
---

Wtyczki umożliwiające wyłączenie liz poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania liz.
...
- `.liz/plugins/` - Wtyczki na poziomie projektu
- `~/.config/liz/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/liz/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/liz/liz.json`)
2. Konfiguracja projektu (`liz.json`)
3. Globalny katalog wtyczek (`~/.config/liz/plugins/`)
4. Katalog wtyczek projektu (`.liz/plugins/`)
...
liz uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z liz do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do liz.
...
:::note
Jeśli korzystasz z aplikacji komputerowej liz, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj liz czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do liz:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod liz. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://liz.ai/docs/sdk).
