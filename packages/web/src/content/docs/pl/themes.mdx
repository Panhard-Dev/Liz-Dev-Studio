...
Dzięki liz możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie liz używa naszego własnego motywu `liz`.
...
liz ma kilka wbudowanych motywów.
...
- Chcą, aby LIZ AI BRASIL pasował do wyglądu ich terminala
...
liz obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/liz/themes/*.json` lub `$XDG_CONFIG_HOME/liz/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.liz/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.liz/themes/*.json`
