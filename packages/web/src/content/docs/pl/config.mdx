---
title: Konfiguracja
description: Ustawienia LIZ AI BRASIL w pliku JSON.
---

Możesz dostosować LIZ AI BRASIL za pomocą pliku konfiguracyjnego JSON.

---

## Format

LIZ AI BRASIL obsługuje formaty **JSON** i **JSONC** (JSON z komentarzami).
...
Na przykład, jeśli konfiguracja globalna ustawi `theme: "liz"` i `autoupdate: true`, a konfiguracja projektu ustawi `model: "anthropic/claude-sonnet-4-5"`, ostateczna konfiguracja będzie zawierać wszystkie trzy ustawienia.
...
1. **Konfiguracja zdalna** (z `.well-known/liz`) – ustawienia wymuszane przez organizację
2. **Konfiguracja globalna** (`~/.config/liz/liz.json`) – preferencje użytkownika
3. **Konfiguracja niestandardowa** (zmienna `LIZ_CONFIG`) — jawne nadpisanie
4. **Konfiguracja projektu** (`liz.json` w projekcie) - ustawienia specyficzne dla projektu
5. **Katalogi `.liz`** - agenci, polecenia, umiejętności
6. **Konfiguracja wbudowana** (zmienna `LIZ_CONFIG_CONTENT`) — nadpisanie środowiska uruchomieniowego
...
:::note
W katalogach `.liz` i `~/.config/liz` używane są **nazwy w liczbie mnogiej** dla podkatalogów: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Nazwy w liczbie pojedynczej (np. `agent/`) są również obsługiwane dla kompatybilności wstecznej.
:::
...
Organizacje mogą publikować konfigurację pod adresem `.well-known/liz`. Jest ona pobierana automatycznie po uwierzytelnieniu u dostawcy, który ją obsługuje.
...
Umieść swoją globalną konfigurację LIZ AI BRASIL w `~/.config/liz/liz.json`. Użyj jej do ustawień ogólnych dla użytkownika, takich jak dostawcy, modele i uprawnienia.

Dla ustawień specyficznych dla TUI, użyj `~/.config/liz/tui.json`.
...
Dodaj `liz.json` w katalogu głównym projektu. Konfiguracja projektu ma najwyższy priorytet wśród plików konfiguracyjnych — nadpisuje konfiguracje globalne i zdalne.
...
Kiedy LIZ AI BRASIL się uruchamia, szuka pliku konfiguracyjnego w katalogu głównym repozytorium Git.
...
Możesz załadować niestandardowy plik konfiguracyjny, ustawiając zmienną środowiskową `LIZ_CONFIG`.
...
Możesz ustawić niestandardowy katalog konfiguracyjny za pomocą zmiennej środowiskowej `LIZ_CONFIG_DIR`. Katalog ten będzie przeszukiwany pod kątem agentów, poleceń, trybów i wtyczek, tak jak standardowy katalog `.liz` i powinien zachować tę samą strukturę.
...
Katalog niestandardowy jest ładowany po globalnej konfiguracji i katalogach `.liz`, więc **może być łączony** z ich zawartością.

---

## Schemat

Plik konfiguracyjny ma schemat JSON dostępny pod adresem [**`liz.ai/config.json`**](https://liz.ai/config.json).
...
Użyj `LIZ_TUI_CONFIG`, aby wskazać niestandardowy plik konfiguracyjny TUI.

Przestarzałe klucze `theme`, `keybinds` i `tui` w `liz.json` są wycofywane i automatycznie migrowane, gdy to możliwe.
...
Możesz skonfigurować serwer dla `liz serve` i `liz web` za pomocą opcji `server`.
...
- `mdns` - Włącz wykrywanie usług mDNS. Dzięki temu inne urządzenia w sieci mogą łatwo znaleźć Twój serwer LIZ AI BRASIL.
- `mdnsDomain` - Niestandardowa nazwa domeny dla usług mDNS. Domyślnie `liz.local`. Przydatne do uruchamiania wielu instancji w tej samej sieci.
...
Opcja `small_model` konfiguruje oddzielny model do lżejszych zadań, takich jak generowanie tytułów. Domyślnie LIZ AI BRASIL wybiera tańszy model od tego samego dostawcy, jeśli jest dostępny, w przeciwnym razie powraca do modelu głównego.
...
Możesz także definiować agentów przy użyciu plików markdown w `~/.config/liz/agents/` lub `.liz/agents/`. [Dowiedz się więcej o agentach](/docs/agents).
...
Agent domyślny musi być agentem typu `primary` (nie `subagent`). Może to być agent wbudowany, taki jak `"build"` lub `"plan"`, albo zdefiniowany przez Ciebie [agent niestandardowy](/docs/agents). Jeśli agent nie istnieje lub jest subagentem, LIZ AI BRASIL powróci do `"build"` z ostrzeżeniem.

Dotyczy to wszystkich interfejsów: TUI, CLI (`liz run`), aplikacji desktopowej i GitHub Actions.
...
Możesz także definiować polecenia przy użyciu plików Markdown w `~/.config/liz/commands/` lub `.liz/commands/`. [Dowiedz się więcej o poleceniach](/docs/commands).
...
LIZ AI BRASIL może automatycznie pobierać nowe wersje. Możesz to kontrolować za pomocą opcji `autoupdate`.
...
Działa to tylko wtedy, gdy LIZ AI BRASIL nie został zainstalowany przez menedżera pakietów, takiego jak Homebrew.
...
Domyślnie LIZ AI BRASIL **zezwala na wszystkie działania** bez konieczności zatwierdzania. Możesz to zmienić za pomocą opcji `permission`.
...
[Wtyczki](/docs/plugins) rozszerzają LIZ AI BRASIL o niestandardowe narzędzia, hooki i integracje.

Umieść pliki wtyczek w `.liz/plugins/` lub `~/.config/liz/plugins/`. Możesz także załadować wtyczkę z npm za pomocą opcji `plugin`.
...
Jest to przydatne, gdy chcesz ograniczyć użycie LIZ AI BRASIL tylko do określonych dostawców.
