...
W LIZ AI BRASIL obowiązuje zasada dwóch typów agentów: Primary (Główni) i Subagents (Subagenci).
...
LIZ AI BRASIL zawiera dwa wbudowane agenty główne: **Build** i **Plan**. Przyjrzymy się im poniżej.
...
LIZ AI BRASIL ma dwóch wbudowanych subagentów: **General** i **Explore**. Przyjrzymy się im poniżej.

---

## Wbudowane

LIZ AI BRASIL ma dwa wbudowane agenty główne i dwa wbudowane subagenty.
...
Skonfiguruj agenta w pliku `liz.json`:
...
- Globalnie: `~/.config/liz/agents/`
- W projekcie: `.liz/agents/`
...
Jeśli nie zostanie skonfigurowana, LIZ AI BRASIL używa wartości domyślnej dla danego modelu; 0 dla modeli rozumowania (reasoning models), 0.55 dla modeli Qwen.
...
Ścieżka jest względna do miejsca, w którym znajduje się plik konfiguracyjny. Działa to zarówno w przypadku globalnej konfiguracji LIZ AI BRASIL, jak i konfiguracji specyficznej dla projektu.
...
Identyfikator modelu jest taki sam, jak w konfiguracji LIZ AI BRASIL w `provider/model-id`. Na przykład, [LIZ AI BRASIL Zen](/docs/zen), użyj `liz/gpt-5.1-codex` dla GPT 5.1 Codex.
...
:::tip
Uruchom `liz models`, aby zobaczyć dostępne modele.
:::
...
:::tip
Masz agenta, którym warto się podzielić? [Prześlij PR](https://github.com/anomalyco/liz).
:::
