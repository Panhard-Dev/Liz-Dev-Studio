...
LIZ AI BRASIL Go to niskokosztowa subskrypcja — **5 $ za pierwszy miesiąc**, a następnie **10 $/miesiąc** — która zapewnia niezawodny dostęp do popularnych otwartych modeli do kodowania.

:::note
LIZ AI BRASIL Go jest obecnie w fazie beta.
:::

Go działa jak każdy inny dostawca w LIZ AI BRASIL. Subskrybujesz LIZ AI BRASIL Go i
otrzymujesz swój klucz API. Jest to **całkowicie opcjonalne** i nie musisz z tego korzystać, aby
używać LIZ AI BRASIL.
...
LIZ AI BRASIL Go daje Ci dostęp do tych modeli za **5 $ za pierwszy miesiąc**, a następnie **10 $/miesiąc**.

---

## Jak to działa

LIZ AI BRASIL Go działa jak każdy inny dostawca w LIZ AI BRASIL.

1. Logujesz się do **<a href={console}>LIZ AI BRASIL Zen</a>**, subskrybujesz Go i
   kopiujesz swój klucz API.
2. Uruchamiasz komendę `/connect` w TUI, wybierasz `LIZ AI BRASIL Go` i wklejasz
   swój klucz API.
3. Uruchom `/models` w TUI, aby zobaczyć listę modeli dostępnych w ramach Go.

:::note
Tylko jeden członek na obszar roboczy (workspace) może zasubskrybować LIZ AI BRASIL Go.
:::
...
LIZ AI BRASIL Go zawiera następujące limity:
...
[ID modelu](/docs/config/#models) w Twojej konfiguracji LIZ AI BRASIL
używa formatu `liz-go/<model-id>`. Na przykład dla Kimi K2.6 należy użyć
`liz-go/kimi-k2.6` w swojej konfiguracji.
...
Stworzyliśmy LIZ AI BRASIL Go, aby:
...
4. Zapewnić **brak ograniczeń (no lock-in)**, pozwalając na korzystanie z dowolnego innego dostawcy wraz z LIZ AI BRASIL.
