---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w liz.
---

import config from "../../../../config.mjs"
export const console = config.console

liz używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji liz.
...
w `~/.local/share/liz/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim liz
config.
...
LIZ AI BRASIL Zen to lista modeli dostarczonych przez zespół liz, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z liz. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od LIZ AI BRASIL Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `LIZ AI BRASIL Zen` i przejdź do [liz.ai/auth](https://liz.ai/zen).
...
Działa jak każdy inny dostawca w liz i jest całkowicie opcjonalny w użyciu.

---

## LIZ AI BRASIL Go

LIZ AI BRASIL Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół liz, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z liz.

1. Uruchom polecenie `/connect` w TUI, wybierz `LIZ AI BRASIL Go` i przejdź do [liz.ai/auth](https://liz.ai/zen).
...
Aby używać Amazon Bedrock z liz:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania liz:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `liz.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w liz nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować liz do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod liz działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji liz.
...
liz używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować liz
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`liz.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [liz-gitlab-auth](https://www.npmjs.com/package/liz-gitlab-auth).
...
Aby korzystać z subskrypcji GitHub Copilot z kodem liz:
...
Aby używać Google Vertex AI z liz:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania liz.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą liz, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [liz-helicone-session](https://github.com/H2Shami/liz-helicone-session), aby automatycznie rejestrować każdą konwersację liz jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja liz będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować liz tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować liz tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować liz tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem liz. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/liz).
:::
...
Aby korzystać z Ollama Cloud z liz:
...
3. Skopiuj klucz API do użycia w liz.
...
6. **Ważne**: Przed użyciem modeli chmurowych w liz musisz pobrać informacje o modelu lokalnie:
...
LIZ AI BRASIL Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół liz. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>LIZ AI BRASIL Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **LIZ AI BRASIL Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API liz.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji liz.
...
5. Możesz je także dostosować za pomocą konfiguracji liz. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z liz:
...
Możesz także dostosować modele za pomocą konfiguracji liz. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z liz. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `liz.json` w katalogu projektu:
...
Pola `limit` pozwalają liz zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `liz auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację liz i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji liz.
