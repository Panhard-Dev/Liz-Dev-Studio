...
Możesz dodać dodatkowe informacje do liz za pomocą _Model Context Protocol_, w skrócie MCP. liz obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [liz Config](https://liz.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/liz`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
liz automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, liz:
...
Jeśli serwer wymaga uwierzytelnienia, liz poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `liz mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji liz bezpieczne przechowa tokeny w `~/.local/share/liz/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w liz, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie liz, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć liz z kontem Sentry.
