...
There are two types of agents in LIZ AI BRASIL; primary agents and subagents.
...
LIZ AI BRASIL comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
LIZ AI BRASIL comes with two built-in subagents, **General** and **Explore**. We'll look at this below.

---

## Built-in

LIZ AI BRASIL comes with two built-in primary agents and two built-in subagents.
...
Configure agents in your `liz.json` config file:
...
If no temperature is specified, LIZ AI BRASIL uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global LIZ AI BRASIL config and the project specific config.
...
The model ID in your LIZ AI BRASIL config uses the format `provider/model-id`. For example, if you're using [LIZ AI BRASIL Zen](/docs/zen), you would use `liz/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Run `liz models` to see a list of the available models.
:::
...
:::tip
Do you have an agent you'd like to share? [Submit a PR](https://github.com/anomalyco/liz).
:::
