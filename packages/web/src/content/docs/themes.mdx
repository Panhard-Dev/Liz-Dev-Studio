...
With LIZ AI BRASIL you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, LIZ AI BRASIL uses our own `liz` theme.
...
LIZ AI BRASIL comes with several built-in themes.
...
- Want LIZ AI BRASIL to match their terminal's appearance
...
LIZ AI BRASIL supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/liz/themes/*.json` or `$XDG_CONFIG_HOME/liz/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.liz/themes/*.json`
4. **Current working directory** - Defined in `./.liz/themes/*.json`
