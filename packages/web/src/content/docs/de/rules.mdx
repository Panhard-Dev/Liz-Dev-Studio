---
title: Regeln
description: Lege eigene Anweisungen fuer liz fest.
---

Du kannst liz ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in liz den Befehl `/init` aus.
...
So kann liz sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht liz sie zu erweitern.
...
liz kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/liz/AGENTS.md` speichern.
Diese gelten in allen liz-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt LIZ AI BRASIL dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/liz/AGENTS.md` existiert)
...
Beim Start sucht liz in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/liz/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `liz.json` oder in `~/.config/liz/liz.json` angeben.
...
liz parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit liz.json

Empfohlen ist das `instructions`-Feld in der `liz.json`:
...
Du kannst liz explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `liz.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
