...
LIZ AI BRASIL bringt eingebaute Tools mit und laesst sich ueber [Custom Tools](/docs/custom-tools) oder [MCP-Server](/docs/mcp-servers) erweitern.
...
Hier sind alle in LIZ AI BRASIL verfuegbaren eingebauten Tools.
...
:::note
Dieses Tool ist nur verfuegbar, wenn `LIZ_EXPERIMENTAL_LSP_TOOL=true` (oder `LIZ_EXPERIMENTAL=true`) gesetzt ist.
:::
...
:::note
Dieses Tool ist nur verfuegbar, wenn der LIZ AI BRASIL-Provider genutzt wird oder die Umgebungsvariable `LIZ_ENABLE_EXA` auf einen 'truthy' Wert (z. B. `true` oder `1`) gesetzt ist.

Zum Aktivieren beim Start von LIZ AI BRASIL:
