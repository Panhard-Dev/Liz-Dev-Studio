...
Sie können externe Tools zu LIZ AI BRASIL hinzufügen, indem Sie das _Model Context Protocol_ oder MCP verwenden. LIZ AI BRASIL unterstützt sowohl lokale als auch Remote-Server.
...
Sie können den MCP-Server in Ihrem [LIZ AI BRASIL Config](https://liz.ai/docs/config/) unter `mcp` definieren. Fügen Sie jedem MCP einen eindeutigen Namen hinzu. Sie können bei der Eingabe von LLM namentlich auf diesen MCP verweisen.
...
Organisationen können über ihren `.well-known/liz`-Endpunkt Standard-MCP-Server bereitstellen. Diese Server sind möglicherweise standardmäßig deaktiviert, sodass Benutzer sich für den Server entscheiden können, die sie benötigen.
...
LIZ AI BRASIL übernimmt automatisch die OAuth-Authentifizierung für Remote-MCP-Server. Wenn ein Server eine Authentifizierung erfordert, wird LIZ AI BRASIL Folgendes tun:
...
Wenn der Server eine Authentifizierung erfordert, werden Sie von LIZ AI BRASIL beim ersten Versuch, ihn zu verwenden, zur Authentifizierung autorisiert. Wenn nicht, können Sie [manually trigger the flow](#authenticating) mit `liz mcp auth <server-name>` verwenden.
...
Der Befehl `mcp auth` öffnet Ihren Browser zur Autorisierung. Nach der Autorisierung speichert LIZ AI BRASIL die Token sicher in `~/.local/share/liz/mcp-auth.json`.
...
Ihre MCPs sind neben integrierten Tools auch als Tools in LIZ AI BRASIL verfügbar. Sie können sie also wie jedes andere Tool über die LIZ AI BRASIL-Konfiguration verwalten.
...
Dadurch wird ein Browserfenster geöffnet, um den OAuth-Ablauf abzuschließen und LIZ AI BRASIL mit Ihrem Sentry-Konto zu verbinden.
