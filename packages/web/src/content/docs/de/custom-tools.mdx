...
description: Erstellen Sie Tools, die der LLM in LIZ AI BRASIL aufrufen kann.
---

Benutzerdefinierte Tools sind von Ihnen erstellte Funktionen, die der LLM während Gesprächen aufrufen kann. Sie arbeiten neben [built-in tools](/docs/tools) von LIZ AI BRASIL wie `read`, `write` und `bash`.
...
- Lokal, indem Sie sie im Verzeichnis `.liz/tools/` Ihres Projekts platzieren.
- Oder global, indem Sie sie in `~/.config/liz/tools/` platzieren.
