---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um LIZ AI BRASIL zu erweitern.
---

Mit Plugins können Sie LIZ AI BRASIL erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von LIZ AI BRASIL zu ändern.
...
- `.liz/plugins/` – Plugins auf Projektebene
- `~/.config/liz/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/liz/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/liz/liz.json`)
2. Projektkonfiguration (`liz.json`)
3. Globales Plugin-Verzeichnis (`~/.config/liz/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.liz/plugins/`)
...
LIZ AI BRASIL führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein LIZ AI BRASIL-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie LIZ AI BRASIL erweitern können.
...
:::note
Wenn Sie die LIZ AI BRASIL-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass LIZ AI BRASIL `.env`-Dateien liest:
...
Plugins können LIZ AI BRASIL auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das LIZ AI BRASIL aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für LIZ AI BRASIL zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://liz.ai/docs/sdk).
