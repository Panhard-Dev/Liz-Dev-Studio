---
title: Anbieter
description: Verwendung eines beliebigen LLM-Anbieters in LIZ AI BRASIL.
---

import config from "../../../../config.mjs"
export const console = config.console

LIZ AI BRASIL verwendet [AI SDK](https://ai-sdk.dev/) und [Models.dev](https://models.dev) zur Unterstützung von **75+ LLM-Anbietern** und unterstützt die Ausführung lokaler Modelle.

Um einen Anbieter hinzuzufügen, müssen Sie:

1. Fügen Sie die API-Schlüssel für den Anbieter mit dem Befehl `/connect` hinzu.
2. Konfigurieren Sie den Anbieter in Ihrer LIZ AI BRASIL-Konfiguration.
...
in `~/.local/share/liz/auth.json`.

---

### Konfiguration

Sie können die Anbieter über den Abschnitt `provider` in Ihrem LIZ AI BRASIL anpassen
config.
...
LIZ AI BRASIL Zen ist eine Liste von Modellen, die vom LIZ AI BRASIL-Team bereitgestellt wurden
Getestet und verifiziert, dass es gut mit LIZ AI BRASIL funktioniert. [Learn more](/docs/zen).

:::tip
Wenn Sie neu sind, empfehlen wir Ihnen, mit LIZ AI BRASIL Zen zu beginnen.
:::

1. Führen Sie den Befehl `/connect` in TUI aus, wählen Sie LIZ AI BRASIL aus und gehen Sie zu [liz.ai/auth](https://liz.ai/auth).
...
Es funktioniert wie jeder andere Anbieter in LIZ AI BRASIL und ist völlig optional.

---

## LIZ AI BRASIL Go

LIZ AI BRASIL Go ist ein kostenguenstiges Abonnement, das zuverlaessigen Zugriff auf beliebte Open-Coding-Modelle bietet, die vom LIZ AI BRASIL-Team getestet und verifiziert wurden, dass sie gut mit LIZ AI BRASIL funktionieren.

1. Führen Sie den Befehl `/connect` in der TUI aus, waehlen Sie `LIZ AI BRASIL Go` und gehen Sie zu [liz.ai/auth](https://liz.ai/zen).
...
So verwenden Sie Amazon Bedrock mit LIZ AI BRASIL:
...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Legen Sie eine dieser Umgebungsvariablen fest, während Sie LIZ AI BRASIL ausführen:

   ```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY liz

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile liz

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX liz
   ```
   ...
   Für eine projektspezifische oder dauerhafte Konfiguration verwenden Sie `liz.json`:
...
:::info
Die Verwendung Ihres Claude Pro/Max-Abonnements in LIZ AI BRASIL wird von [Anthropic](https://anthropic.com) nicht offiziell unterstützt.
:::
...
Sie können liz so konfigurieren, dass es lokale Modelle über [Atomic Chat](https://atomic.chat) verwendet — eine Desktop-Anwendung, die lokale LLMs hinter einem OpenAI-kompatiblen API-Server bereitstellt (Standard-Endpunkt `http://127.0.0.1:1337/v1`).

```json title="liz.json" "atomic-chat" {5, 6, 8, 10-14}
{
  "$schema": "https://liz.ai/config.json",
  "provider": {
    "atomic-chat": {
...
2. Gehen Sie zu [Azure AI Foundry](https://ai.azure.com/) und stellen Sie ein Modell bereit.
   ...
   :::note
   Der Bereitstellungsname muss mit dem Modellnamen übereinstimmen, damit LIZ AI BRASIL ordnungsgemäß funktioniert.
   :::
...
5. Legen Sie Ihren Ressourcennamen als Umgebungsvariable fest:
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX liz
   ```
...
5. Führen Sie den Befehl `/models` aus, um ein Modell auszuwählen.
   ...
   Sie können Modelle auch über Ihre LIZ AI BRASIL-Konfiguration hinzufügen.
...
2. Wählen Sie Ihre Authentifizierungsmethode:
   ...
   3. Name: `LIZ AI BRASIL`, Bereiche: `api`
...
um das Token im LIZ AI BRASIL-Authentifizierungsspeicher zu speichern.
:::

##### Selbstgehostetes GitLab

:::note[Compliance-Hinweis]
LIZ AI BRASIL verwendet ein kleines Modell für einige AI-Aufgaben wie das Generieren des Sitzungstitels.
Es ist standardmäßig für die Verwendung von gpt-5-nano konfiguriert und wird von Zen gehostet. Zum Sperren von LIZ AI BRASIL
Um nur Ihre eigene von GitLab gehostete Instanz zu verwenden, fügen Sie Folgendes zu Ihrer hinzu
`liz.json`-Datei. Es wird außerdem empfohlen, die Sitzungsfreigabe zu deaktivieren.
...
Weitere Dokumentation auf der [liz-gitlab-auth](https://www.npmjs.com/package/liz-gitlab-auth)-Homepage.

##### Konfiguration

Anpassen über `liz.json`:
...
So verwenden Sie Ihr GitHub Copilot-Abonnement mit LIZ AI BRASIL:
...
So verwenden Sie Google Vertex AI mit LIZ AI BRASIL:
...
2. Legen Sie die erforderlichen Umgebungsvariablen fest:
   ...
   Legen Sie sie fest, während Sie LIZ AI BRASIL ausführen.

   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id liz
   ```
...
Falls Sie eine Funktion oder ein Modell von Helicone sehen, das nicht automatisch über LIZ AI BRASIL konfiguriert wird, können Sie es jederzeit selbst konfigurieren.
...
```jsonc title="~/.config/liz/liz.jsonc"
{
  "$schema": "https://liz.ai/config.json",
  "provider": {
    "helicone": {
...
Mit der [Sessions](https://docs.helicone.ai/features/sessions)-Funktion von Helicone können Sie zusammengehörige LLM-Anfragen gruppieren. Verwenden Sie das [liz-helicone-session](https://github.com/H2Shami/liz-helicone-session)-Plugin, um jede LIZ AI BRASIL-Konversation automatisch als Sitzung in Helicone zu protokollieren.

```bash
npm install -g liz-helicone-session
```
...
```json title="liz.json"
{
  "plugin": ["liz-helicone-session"]
}
```

Das Plugin fügt `Helicone-Session-Id`- und `Helicone-Session-Name`-Header in Ihre Anfragen ein. Auf der Seite „Sitzungen“ von Helicone wird jede LIZ AI BRASIL-Konversation als separate Sitzung aufgeführt.
...
Sie können LIZ AI BRASIL für die Verwendung lokaler Modelle über das Dienstprogramm [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server konfigurieren

```json title="liz.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://liz.ai/config.json",
  "provider": {
    "llama.cpp": {
...
Sie können LIZ AI BRASIL über LM Studio für die Verwendung lokaler Modelle konfigurieren.

```json title="liz.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://liz.ai/config.json",
  "provider": {
    "lmstudio": {
...
Sie können LIZ AI BRASIL über Ollama für die Verwendung lokaler Modelle konfigurieren.

:::tip
Ollama kann sich automatisch für LIZ AI BRASIL konfigurieren. Weitere Informationen finden Sie unter [Ollama integration docs](https://docs.ollama.com/integrations/liz).
:::

```json title="liz.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://liz.ai/config.json",
  "provider": {
    "ollama": {
...
So verwenden Sie Ollama Cloud mit LIZ AI BRASIL:
...
3. Kopieren Sie den API-Schlüssel zur Verwendung in LIZ AI BRASIL.
...
6. **Wichtig**: Bevor Sie Cloud-Modelle in LIZ AI BRASIL verwenden, müssen Sie die Modellinformationen lokal abrufen:
...
LIZ AI BRASIL Zen ist eine Liste getesteter und verifizierter Modelle, die vom LIZ AI BRASIL-Team bereitgestellt werden. [Learn more](/docs/zen).

1. Melden Sie sich bei **<a href={console}>LIZ AI BRASIL Zen</a>** an und klicken Sie auf **Create API Key**.

2. Führen Sie den Befehl `/connect` aus und suchen Sie nach **LIZ AI BRASIL Zen**.

   ```txt
   /connect
   ```

3. Geben Sie Ihren LIZ AI BRASIL API-Schlüssel ein.
...
4. Viele OpenRouter-Modelle sind standardmäßig vorinstalliert. Führen Sie den Befehl `/models` aus, um das gewünschte Modell auszuwählen.
   ...
   Sie können auch zusätzliche Modelle über Ihre LIZ AI BRASIL-Konfiguration hinzufügen.
...
5. Sie können sie auch über Ihre LIZ AI BRASIL-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe eines Anbieters
...
3. Geben Sie Ihren JSON-Service-Account-Key ein.
   ...
   ```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' liz
   ```
...
4. Legen Sie optional die Bereitstellung ID und die Ressourcengruppe fest:
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group liz
   ```
...
So verwenden Sie [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) mit LIZ AI BRASIL:
...
Sie können Modelle auch über Ihre LIZ AI BRASIL-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe der Provider-Routing-Reihenfolge.
...
:::tip
Sie können jeden OpenAI-kompatiblen Anbieter mit LIZ AI BRASIL verwenden. Die meisten modernen AI-Anbieter bieten OpenAI-kompatible APIs an.
:::
...
3. Geben Sie Ihren API-Schlüssel für den Anbieter ein.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in liz.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Erstellen oder aktualisieren Sie Ihre `liz.json`-Datei in Ihrem Projektverzeichnis:

   ```json title="liz.json" ""myprovider"" {5-15}
   {
     "$schema": "https://liz.ai/config.json",
     "provider": {
       "myprovider": {
...
```json title="liz.json" {9,11,17-20}
{
  "$schema": "https://liz.ai/config.json",
  "provider": {
    "myprovider": {
...
Mithilfe der `limit`-Felder kann LIZ AI BRASIL erkennen, wie viel Kontext Sie noch haben. Standardanbieter beziehen diese automatisch von models.dev.
...
1. **Überprüfen Sie die Authentifizierungseinrichtung**: Führen Sie `liz auth list` aus, um zu sehen, ob die Anmeldeinformationen vorhanden sind
...
2. Überprüfen Sie bei benutzerdefinierten Anbietern die LIZ AI BRASIL-Konfiguration und:
   - Stellen Sie sicher, dass der im Befehl `/connect` verwendete Anbieter ID mit dem ID in Ihrer LIZ AI BRASIL-Konfiguration übereinstimmt.
