...
Agent Skills erlauben LIZ AI BRASIL, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
LIZ AI BRASIL sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft LIZ AI BRASIL vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.liz/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/liz/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.liz/skills/git-release/SKILL.md` zum Beispiel so:
...
LIZ AI BRASIL listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `liz.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `liz.json`):
