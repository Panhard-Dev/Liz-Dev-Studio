...
Es gibt zwei Arten von Agenten in LIZ AI BRASIL; Primäragenten und Subagenten.
...
LIZ AI BRASIL verfügt über zwei integrierte Primäragenten: **Build** und **Plan**. Auch
...
Konfigurieren Sie Agenten in Ihrer `liz.json`-Konfigurationsdatei:
...
Wenn keine Temperatur angegeben ist, verwendet LIZ AI BRASIL modellspezifische Standardwerte; typischerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle.
...
Dieser Pfad ist relativ zum Speicherort der Konfigurationsdatei. Dies funktioniert sowohl für die globale LIZ AI BRASIL-Konfiguration als auch für die projektspezifische Konfiguration.
...
Die Modell-ID in Ihrer LIZ AI BRASIL-Konfiguration verwendet das Format `provider/model-id`. Wenn Sie beispielsweise [LIZ AI BRASIL Zen](/docs/zen) verwenden, würden Sie `liz/gpt-5.1-codex` für GPT 5.1 Codex verwenden.
...
:::tip
Führen Sie `liz models` aus, um eine Liste der verfügbaren Modelle anzubieten.
:::
...
:::tip
Haben Sie einen Agenten, den Sie gerne teilen möchten? [Reichen Sie einen PR ein](https://github.com/anomalyco/liz).
:::
