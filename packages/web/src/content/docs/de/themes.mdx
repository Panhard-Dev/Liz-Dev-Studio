...
Mit LIZ AI BRASIL kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet LIZ AI BRASIL das `liz`-Theme.
...
LIZ AI BRASIL liefert mehrere eingebaute Themes mit.
...
- LIZ AI BRASIL optisch ans Terminal anpassen wollen
...
LIZ AI BRASIL unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/liz/themes/*.json` oder `$XDG_CONFIG_HOME/liz/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.liz/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.liz/themes/*.json`
