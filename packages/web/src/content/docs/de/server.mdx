---
title: Server
description: Nutze den liz-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `liz serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein liz-Client nutzen kann.
...
Setze `LIZ_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `liz`, kann aber mit `LIZ_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `liz serve` und `liz web`.
...
Wenn du `liz` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den liz-Server, um liz programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `liz serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `liz serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die LIZ AI BRASIL-[IDE](/docs/ide)-Plugins.
...
Der liz-Server stellt folgende APIs bereit.
