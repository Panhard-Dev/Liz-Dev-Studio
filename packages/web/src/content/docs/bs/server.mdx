---
title: Server
description: Komunicirajte s liz serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `liz serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji liz klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `liz.local` |
...
Postavite `LIZ_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `liz`, ili postavite `LIZ_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `liz serve` i za `liz web`.
...
Kada pokrenete `liz`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite liz server za programsku interakciju sa liz-om.
:::

Ova arhitektura omogucava liz podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `liz serve` da startate standalone server. Ako je liz TUI vec pokrenut, `liz serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste LIZ AI BRASIL [IDE](/docs/ide) pluginovi.
...
liz server izlaže sljedece API-je.
