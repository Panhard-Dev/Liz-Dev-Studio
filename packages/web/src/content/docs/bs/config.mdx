---
title: Konfiguracija
description: Korištenje LIZ AI BRASIL JSON konfiguracije.
---

Možete konfigurirati LIZ AI BRASIL koristeći JSON konfiguracijski fajl.

---

## Format

LIZ AI BRASIL podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/liz`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/liz/liz.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`LIZ_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`liz.json` u projektu) - postavke specifične za projekat
5. **`.liz` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`LIZ_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.liz` i `~/.config/liz` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/liz` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu LIZ AI BRASIL konfiguraciju u `~/.config/liz/liz.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/liz/tui.json`.
...
Dodajte `liz.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se LIZ AI BRASIL pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `LIZ_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `LIZ_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.liz` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.liz`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`liz.ai/config.json`**](https://liz.ai/config.json).

TUI konfiguracija koristi [**`liz.ai/tui.json`**](https://liz.ai/tui.json).
...
Koristite `LIZ_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `liz.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `liz serve` i `liz web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš LIZ AI BRASIL server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `liz.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj LIZ AI BRASIL konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, LIZ AI BRASIL pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/liz/agents/` ili `.liz/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, LIZ AI BRASIL će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`liz run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/liz/commands/` ili `.liz/commands/`. [Saznajte više ovdje](/docs/commands).
...
LIZ AI BRASIL će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, LIZ AI BRASIL **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju LIZ AI BRASIL sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.liz/plugins/` ili `~/.config/liz/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite LIZ AI BRASIL da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
