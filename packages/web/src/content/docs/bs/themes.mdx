...
U LIZ AI BRASIL mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, LIZ AI BRASIL koristi nasu `liz` temu.
...
LIZ AI BRASIL dolazi sa vise ugradenih tema.
...
- Zele da LIZ AI BRASIL odgovara izgledu njihovog terminala
...
LIZ AI BRASIL podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/liz/themes/*.json` ili `$XDG_CONFIG_HOME/liz/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.liz/themes/*.json`
4. **Trenutni radni direktorij** - `./.liz/themes/*.json`
