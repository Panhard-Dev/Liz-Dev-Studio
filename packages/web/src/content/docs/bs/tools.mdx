...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. LIZ AI BRASIL dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u LIZ AI BRASIL.
...
:::note
Ovaj alat je dostupan samo kada je `LIZ_EXPERIMENTAL_LSP_TOOL=true` (ili `LIZ_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz LIZ AI BRASIL provajdera ili kada je varijabla `LIZ_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju LIZ AI BRASIL:
