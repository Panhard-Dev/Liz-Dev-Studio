...
Možete dodati vanjske alate u LIZ AI BRASIL koristeći _Model Context Protocol_, ili MCP. LIZ AI BRASIL podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [LIZ AI BRASIL Config](https://liz.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/liz`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
LIZ AI BRASIL automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, LIZ AI BRASIL će:
...
Ako server zahtijeva autentifikaciju, LIZ AI BRASIL će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `liz mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, LIZ AI BRASIL sigurno čuva tokene u `~/.local/share/liz/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u LIZ AI BRASIL, zajedno s ugrađenim alatima. Možete njima upravljati kroz LIZ AI BRASIL konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete LIZ AI BRASIL sa vašim Sentry nalogom.
