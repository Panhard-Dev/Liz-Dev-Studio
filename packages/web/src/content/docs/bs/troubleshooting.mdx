...
Da biste otklonili probleme s LIZ AI BRASIL, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\liz\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `liz --log-level DEBUG`.

---

## Pohrana

LIZ AI BRASIL pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/liz/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\liz`
...
LIZ AI BRASIL Desktop pokreće lokalni LIZ AI BRASIL server (`liz-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `LIZ AI BRASIL` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/liz/liz.jsonc` (ili `~/.config/liz/liz.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/liz/liz.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\liz\liz.jsonc`
...
LIZ AI BRASIL također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/liz/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\liz\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga LIZ AI BRASIL mogao ponovo izgraditi.

1. Potpuno zatvorite LIZ AI BRASIL Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/liz`
- **Linux**: obrišite `~/.cache/liz` (ili pokrenite `rm -rf ~/.cache/liz`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\liz`

3. Ponovo pokrenite LIZ AI BRASIL Desktop.

---

### Rješavanje problema sa vezom na serveru

LIZ AI BRASIL Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `liz.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `LIZ_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `LIZ_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, LIZ AI BRASIL Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa LIZ AI BRASIL karakteristikama.

---

### Obavještenja se ne prikazuju

LIZ AI BRASIL Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za LIZ AI BRASIL u postavkama vašeg OS-a, i
...
1. Zatvorite LIZ AI BRASIL Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka LIZ AI BRASIL Desktop aplikacije):

- `liz.settings.dat` (URL zadanog servera za desktop)
- `liz.global.dat` i `liz.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Ako imate problema s LIZ AI BRASIL:
...
Da saznate kojim modelima imate pristup, pokrenite `liz models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\liz`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. LIZ AI BRASIL dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\liz`

2. Ponovo pokrenite LIZ AI BRASIL da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti LIZ AI BRASIL da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
LIZ AI BRASIL će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
