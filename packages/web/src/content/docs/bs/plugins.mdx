---
title: Dodaci
description: Napišite vlastite dodatke za proširenje LIZ AI BRASIL.
---

Dodaci vam omogućavaju da proširite LIZ AI BRASIL spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja LIZ AI BRASIL.
...
- `.liz/plugins/` - Dodaci na nivou projekta
- `~/.config/liz/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/liz/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/liz/liz.json`)
2. Konfiguracija projekta (`liz.json`)
3. Globalni direktorij dodataka (`~/.config/liz/plugins/`)
4. Direktorij dodataka projekta (`.liz/plugins/`)
...
LIZ AI BRASIL pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: Liz SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje LIZ AI BRASIL.
...
Spriječite liz da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u liz:
...
Pomoćnik `tool` kreira prilagođeni alat koji liz može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za liz zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://liz.ai/docs/sdk) za detalje.
