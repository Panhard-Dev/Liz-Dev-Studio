---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u LIZ AI BRASIL.
---

import config from "../../../../config.mjs"
export const console = config.console

LIZ AI BRASIL koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj LIZ AI BRASIL konfiguraciji.
...
u `~/.local/share/liz/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem LIZ AI BRASIL
config.
...
LIZ AI BRASIL Zen je lista modela koje je obezbedio LIZ AI BRASIL tim koji su bili
testirano i potvrđeno da dobro radi sa LIZ AI BRASIL. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa LIZ AI BRASIL Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite liz i idite na [liz.ai/auth](https://liz.ai/auth).
...
Radi kao i svaki drugi provajder u LIZ AI BRASIL i potpuno je opcionalan za korištenje.

---

## LIZ AI BRASIL Go

LIZ AI BRASIL Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža LIZ AI BRASIL tim i koji su testirani i verificirani da dobro rade s LIZ AI BRASIL-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `LIZ AI BRASIL Go` i idite na [liz.ai/auth](https://liz.ai/zen).
...
Da biste koristili Amazon Bedrock s LIZ AI BRASIL:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete liz:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `liz.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u LIZ AI BRASIL.
:::
...
Možete konfigurirati liz za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi liz ispravno radio.
   :::
...
Također možete dodati modele kroz svoju liz konfiguraciju.
...
3.  Naziv: `LIZ AI BRASIL`, opseg: `api`
...
da pohrani token u liz auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
LIZ AI BRASIL koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate LIZ AI BRASIL
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`liz.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [liz-gitlab-auth](https://www.npmjs.com/package/liz-gitlab-auth).

##### Konfiguracija

Prilagodite putem `liz.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s liz:
...
Za korištenje Google Vertex AI s LIZ AI BRASIL:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće liz.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem liza, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [liz-helicone-session](https://github.com/H2Shami/liz-helicone-session) da automatski prijavite svaki LIZ AI BRASIL razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki LIZ AI BRASIL razgovor naveden kao zasebna sesija.
...
Možete konfigurirati liz za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati liz za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati liz za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za LIZ AI BRASIL. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/liz) za detalje.
:::
...
Da biste koristili Ollama Cloud s LIZ AI BRASIL:
...
3. Kopirajte API ključ za korištenje u LIZ AI BRASIL.
...
6. **Važno**: Prije upotrebe modela oblaka u LIZ AI BRASIL, morate lokalno povući informacije o modelu:
...
LIZ AI BRASIL Zen je lista testiranih i verifikovanih modela koju je obezbedio LIZ AI BRASIL tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>LIZ AI BRASIL Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **LIZ AI BRASIL Zen**.

```txt
   /connect
```

3. Unesite svoj LIZ AI BRASIL API ključ.
...
Također možete dodati dodatne modele putem vaše liz konfiguracije.
...
5. Također ih možete prilagoditi putem vaše liz konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa Lizom:
...
Također možete prilagoditi modele kroz svoju liz konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s liz-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `liz.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju LIZ AI BRASIL da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `liz auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite LIZ AI BRASIL konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj liz konfiguraciji.
