---
title: Plugins
description: Skriv dine egne plugins for at udvide LIZ AI BRASIL.
---

Plugins giver dig mulighed for at udvide LIZ AI BRASIL ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre LIZ AI BRASILs standardadfærd.
...
- `.liz/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/liz/node_modules/`.
...
1. Global konfiguration (`~/.config/liz/liz.json`)
2. Projektkonfiguration (`liz.json`)
3. Global plugin-mappe (`~/.config/liz/plugins/`)
4. Projekt plugin bibliotek (`.liz/plugins/`)
...
LIZ AI BRASIL kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En liz SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide liz.
...
:::note
Hvis du bruger LIZ AI BRASIL desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Plugins kan også tilføje brugerdefinerede værktøjer til liz:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som liz kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for liz sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://liz.ai/docs/sdk) for detaljer.
