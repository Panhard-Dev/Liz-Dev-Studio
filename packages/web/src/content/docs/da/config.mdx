---
title: Konfiguration
description: Ved at bruge LIZ AI BRASIL JSON konfig.
---

Du kan konfigurere LIZ AI BRASIL ved hjælp af en JSON-konfigurationsfil.

---

## Format

LIZ AI BRASIL understøtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
1. **Fjernkonfiguration** (fra `.well-known/liz`) - organisatoriske standardindstillinger
2. **Global config** (`~/.config/liz/liz.json`) - brugerpræferencer
3. **Tilpasset konfiguration** (`LIZ_CONFIG` env var) - tilpassede tilsidesættelser
4. **Project config** (`liz.json` i projekt) - projektspecifikke indstillinger
5. **`.liz` mapper** - agent, kommandoer, plugins
6. **Inline config** (`LIZ_CONFIG_CONTENT` env var) - runtime tilsidesættelser
...
:::note
`.liz` og `~/.config/liz` mapperne bruger **flertalsnavne** for undermapper: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkelte navne (f.eks. `agent/`) understøtter også bagudkompatibilitet.
:::
...
Organisationer kan levere standardkonfiguration via `.well-known/liz`-slutpunktet. Dette hentes automatisk, når du godkender med en udbyder, der understøtter det.
...
Placer din globale LIZ AI BRASIL-konfiguration i `~/.config/liz/liz.json`. Brug global konfiguration til brugerdækkende præferencer som temaer, udbydere eller nøglebindinger.
...
Tilføj `liz.json` i dit projektrod. Project config har den højeste forrang blandt standard config-filer - den tilsidesætter både globale og eksterne config.

:::tip
Placer projektspecifik konfiguration i roden af ​​dit projekt.
:::

Når LIZ AI BRASIL starter op, søger den efter en konfigurationsfil i den aktuelle mappe eller går op til den nærmeste Git-mappe.
...
Angiv en brugerdefineret konfigurationsfilsti ved hjælp af miljøvariablen `LIZ_CONFIG`.
...
Angiv en brugerdefineret konfigurationsmappe ved hjælp af `LIZ_CONFIG_DIR`
miljøvariabel. Dette kort vil blive søgt efter agenter, kommandoer,
modes og plugins ligesom standard `.liz` mappen, og bør
...
Den brugerdefinerede mappe indlæses efter den globale konfig og `.liz` mapper, så den **kan tilsidesætte** deres indstillinger.

---

## Skema

Konfigurationsfilen har et skema, der defineres i [**`liz.ai/config.json`**](https://liz.ai/config.json).
...
Du kan konfigurere serverindstillinger for kommandoerne `liz serve` og `liz web` gennem indstillingen `server`.
...
- `mdns` - Aktiver mDNS-tjenesteopdagelse. Dette gør det muligt for andre enheder på netværket at opdage din LIZ AI BRASIL-server.
- `mdnsDomain` - Brugerdefineret domænenavn til mDNS-tjeneste. Som standard er `liz.local`. Nyttigt til at køre flere forekomster på det samme netværk.
...
Du kan konfigurere de udbydere og modeller, du vil bruge i din LIZ AI BRASIL-konfiguration, gennem mulighederne `provider`, `model` og `small_model`.
...
Indstillingen `small_model` konfigurerer en separat model til lette opgaver som titelgenerering. Som standard forsøger LIZ AI BRASIL at bruge en billigere model, hvis en er tilgængelig fra din udbyder, ellers falder den tilbage til din hovedmodel.
...
Du kan konfigurere det tema, du vil bruge i din LIZ AI BRASIL-konfiguration, gennem indstillingen `theme`.
...
Du kan også definere agenter ved at bruge markdown-filer i `~/.config/liz/agents/` eller `.liz/agents/`. [Learn more here](/docs/agents).
...
Standardagenten skal være en primær agent (ikke en subagent). Dette kan være en indbygget agent som `"build"` eller `"plan"`, eller en [custom agent](/docs/agents), du har defineret. Hvis den angivne agent ikke eksisterer eller er en underagent, vil LIZ AI BRASIL falde tilbage til `"build"` med en advarsel.

Denne indstilling gælder på tværs af alle grænseflader: TUI, CLI (`liz run`), desktop-app og GitHub Action.
...
Du kan også definere kommandoer ved hjælp af markdown-filer i `~/.config/liz/commands/` eller `.liz/commands/`. [Learn more here](/docs/commands).
...
LIZ AI BRASIL vil automatisk downloade alle nye opdateringer, når den starter op. Du kan deaktivere dette med indstillingen `autoupdate`.
...
Som standard liz **tillader alle operationer** uden at kræve eksplicit godkendelse. Du kan ændre dette ved at bruge indstillingen `permission`.
...
[Plugins](/docs/plugins) udvide LIZ AI BRASIL med brugerdefinerede værktøjer, kroge og integrationer.

Placer plugin-filer i `.liz/plugins/` eller `~/.config/liz/plugins/`. Du kan også indlæse plugins fra npm gennem indstillingen `plugin`.
...
Dette er nyttigt, når du vil begrænse LIZ AI BRASIL til kun at bruge specifikke udbydere i stedet for at deaktivere dem én efter én.
