...
Du kan tilføje eksterne værktøjer til LIZ AI BRASIL ved hjælp af _Model Context Protocol_ eller MCP. LIZ AI BRASIL understøtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i din [LIZ AI BRASIL Config](https://liz.ai/docs/config/) under `mcp`. Tilføj hver MCP med et unikt navn. Du kan henvise til denne MCP ved navn, når du spørger LLM.
...
Organisationer kan levere standard MCP-servere via deres `.well-known/liz`-slutpunkt. Disse servere kan være deaktiveret som standard, hvilket giver brugerne mulighed for at tilmelde sig dem, de har brug for.
...
LIZ AI BRASIL håndterer automatisk OAuth-godkendelse for eksterne MCP-servere. Når en server kræver godkendelse, vil LIZ AI BRASIL:
...
Hvis serveren kræver godkendelse, vil LIZ AI BRASIL bede dig om at godkende, når du prøver at bruge den første gang. Hvis ikke, kan du [manually trigger the flow](#authenticating) med `liz mcp auth <server-name>`.
...
Kommandoen `mcp auth` åbner din browser for godkendelse. Når du har godkendt, vil LIZ AI BRASIL opbevare tokens sikkert i `~/.local/share/liz/mcp-auth.json`.
...
Dine MCPs er tilgængelige som værktøjer i **TK\_** sammen med indbyggede værktøjer. Så du kan administrere dem gennem LIZ AI BRASIL-konfigurationen som ethvert andet værktøj.
...
Dette åbner et browservindue for at fuldføre OAuth-flowet og forbinde LIZ AI BRASIL til din Sentry-konto.
