---
title: Server
description: Interager med liz-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `liz serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en liz-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `liz.local` |
...
Indstil `LIZ_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `liz`, eller indstil `LIZ_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `liz serve` og `liz web`.
...
Når du kører `liz` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug liz-serveren til at interagere med liz programmatisk.
:::

Denne arkitektur lader liz understøtte flere klienter og lader dig interagere med liz programmatisk.

Du kan køre `liz serve` for at starte en selvstændig server. Hvis du har
liz TUI kørende, vil `liz serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af LIZ AI BRASIL [IDE](/docs/ide) plugins.
...
LIZ AI BRASIL-serveren viser følgende API'er.
