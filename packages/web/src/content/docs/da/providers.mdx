---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i LIZ AI BRASIL.
---

import config from "../../../../config.mjs"
export const console = config.console

LIZ AI BRASIL bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i LIZ AI BRASIL-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/liz/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i LIZ AI BRASIL-konfigurationen.
...
LIZ AI BRASIL Zen er en liste over modeller leveret af LIZ AI BRASIL-teamet, som er blevet testet og verificeret til at fungere godt med LIZ AI BRASIL. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med LIZ AI BRASIL Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg liz og gå til [liz.ai/auth](https://liz.ai/auth).
...
Det fungerer som alle andre udbydere i LIZ AI BRASIL og er helt valgfrit at bruge.

---

## LIZ AI BRASIL Go

LIZ AI BRASIL Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af LIZ AI BRASIL-teamet, som er testet og verificeret til at fungere godt med LIZ AI BRASIL.

1. Kør kommandoen `/connect` i TUI, vælg `LIZ AI BRASIL Go`, og gå til [liz.ai/auth](https://liz.ai/zen).
...
Sådan bruger du Amazon Bedrock med LIZ AI BRASIL:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører liz:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `liz.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i LIZ AI BRASIL understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere liz til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at liz skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem liz-konfigurationen.
...
gemme tokenet i liz auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
LIZ AI BRASIL bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse LIZ AI BRASIL
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`liz.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [liz-gitlab-auth](https://www.npmjs.com/package/liz-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `liz.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med LIZ AI BRASIL:
...
Sådan bruger du Google Vertex AI med LIZ AI BRASIL:
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem liz, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [liz-helicone-session](https://github.com/H2Shami/liz-helicone-session) plugin for automatisk at logge hver LIZ AI BRASIL-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver LIZ AI BRASIL-samtale opført som en separat session.
...
Du kan konfigurere liz til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere liz til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere liz til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for LIZ AI BRASIL. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/liz) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med LIZ AI BRASIL:
...
3. Kopier API-nøglen til brug i LIZ AI BRASIL.
...
6. **Vigtigt**: Før du bruger skymodeller i LIZ AI BRASIL, skal du hente modelinformationen lokalt:
...
LIZ AI BRASIL Zen er en liste over testede og verificerede modeller leveret af LIZ AI BRASIL-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>LIZ AI BRASIL Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **LIZ AI BRASIL Zen**.

   ```txt
   /connect
   ```

3. Indtast LIZ AI BRASIL API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem liz-konfigurationen.
...
5. Du kan også tilpasse dem gennem liz-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med LIZ AI BRASIL:
...
Du kan også tilpasse modeller gennem liz-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med liz. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `liz.json`-filen i projektkataloget:
...
`limit`-felterne lader LIZ AI BRASIL forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `liz auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek liz-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i liz-konfigurationen.
