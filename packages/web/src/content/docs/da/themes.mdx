...
Med LIZ AI BRASIL kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger LIZ AI BRASIL vårt eget `liz`-tema.
...
LIZ AI BRASIL støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/liz/themes/*.json` eller `$XDG_CONFIG_HOME/liz/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.liz/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.liz/themes/*.json`
