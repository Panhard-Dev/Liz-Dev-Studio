---
title: Provider
description: Usa qualsiasi provider LLM in LIZ AI BRASIL.
---

import config from "../../../../config.mjs"
export const console = config.console

LIZ AI BRASIL usa [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) per supportare **oltre 75 provider LLM** e anche modelli locali.

Per aggiungere un provider devi:

1. Aggiungere le API key del provider con il comando `/connect`.
2. Configurare il provider nella configurazione di LIZ AI BRASIL.
...
configurazione di LIZ AI BRASIL.
...
LIZ AI BRASIL Zen e una lista di modelli forniti dal team LIZ AI BRASIL, testati e
verificati per funzionare bene con LIZ AI BRASIL. [Scopri di piu](/docs/zen).

:::tip
Se sei all'inizio, ti consigliamo di partire da LIZ AI BRASIL Zen.
:::

1. Esegui `/connect` nella TUI, seleziona liz e vai su [liz.ai/auth](https://liz.ai/auth).
...
Funziona come qualsiasi altro provider in LIZ AI BRASIL ed e totalmente opzionale.
...
Per usare Amazon Bedrock con LIZ AI BRASIL:
...
2. **Configura l'autenticazione** usando uno dei seguenti metodi:
   ...
   ***
   ...
   Imposta una di queste variabili d'ambiente mentre esegui liz:
   ...
   ***
   ...
   Per configurazione specifica del progetto o persistente, usa `liz.json`:
...
:::info
L'uso dell'abbonamento Claude Pro/Max in LIZ AI BRASIL non è ufficialmente supportato da [Anthropic](https://anthropic.com).
:::
...
Puoi configurare liz per utilizzare modelli locali tramite [Atomic Chat](https://atomic.chat), un'applicazione desktop che esegue LLM locali dietro un server API compatibile OpenAI (endpoint predefinito `http://127.0.0.1:1337/v1`).
...
2. Vai su [Azure AI Foundry](https://ai.azure.com/) e fai il deploy di un modello.
   ...
   :::note
   Il nome del deployment deve corrispondere al nome del modello affinché liz funzioni correttamente.
   :::
...
5. Esegui il comando `/models` per selezionare un modello.
   ...
   Puoi anche aggiungere modelli tramite la tua configurazione di liz.
...
2. Scegli il tuo metodo di autenticazione:
   ...
   3. Nome: `LIZ AI BRASIL`, Scopes: `api`
...
salvare il token nello storage di auth di liz.
:::

##### GitLab Self-Hosted

:::note[nota di compliance]
LIZ AI BRASIL usa un modello piccolo per alcuni task AI come generare il titolo della sessione.
È configurato per usare gpt-5-nano di default, ospitato da Zen. Per bloccare LIZ AI BRASIL
a usare solo la tua istanza GitLab self-hosted, aggiungi quanto segue al tuo
file `liz.json`. Si raccomanda anche di disabilitare la condivisione sessioni.
...
Maggior documentazione sulla homepage di [liz-gitlab-auth](https://www.npmjs.com/package/liz-gitlab-auth).

##### Configurazione

Personalizza tramite `liz.json`:
...
Per usare il tuo abbonamento GitHub Copilot con liz:
...
Per usare Google Vertex AI con LIZ AI BRASIL:
...
Nel caso vedessi una funzionalità o un modello di Helicone che non viene configurato automaticamente da liz, puoi sempre configurarlo tu stesso.
...
La funzionalità [Sessions](https://docs.helicone.ai/features/sessions) di Helicone ti permette di raggruppare richieste LLM correlate. Usa il plugin [liz-helicone-session](https://github.com/H2Shami/liz-helicone-session) per loggare automaticamente ogni conversazione di LIZ AI BRASIL come una sessione in Helicone.
...
Il plugin inietta gli header `Helicone-Session-Id` e `Helicone-Session-Name` nelle tue richieste. Nella pagina Sessions di Helicone, vedrai ogni conversazione di LIZ AI BRASIL elencata come sessione separata.
...
Puoi configurare liz per usare modelli locali tramite l'utility llama-server di [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puoi configurare liz per usare modelli locali tramite LM Studio.
...
Puoi configurare liz per usare modelli locali tramite Ollama.

:::tip
Ollama può auto-configurarsi per LIZ AI BRASIL. Vedi la [documentazione di integrazione Ollama](https://docs.ollama.com/integrations/liz) per dettagli.
:::
...
Per usare Ollama Cloud con LIZ AI BRASIL:
...
3. Copia la chiave API da usare in LIZ AI BRASIL.
...
6. **Importante**: Prima di usare modelli cloud in LIZ AI BRASIL, devi scaricare le informazioni del modello localmente:
...
LIZ AI BRASIL Zen è una lista di modelli testati e verificati forniti dal team LIZ AI BRASIL. [Scopri di più](/docs/zen).

1. Accedi a **<a href={console}>LIZ AI BRASIL Zen</a>** e clicca **Create API Key**.

2. Esegui il comando `/connect` e cerca **LIZ AI BRASIL Zen**.

   ```txt
   /connect
   ```

3. Inserisci la tua chiave API di LIZ AI BRASIL.
...
4. Molti modelli OpenRouter sono precaricati di default, esegui il comando `/models` per selezionare quello che vuoi.
   ...
   Puoi anche aggiungere modelli addizionali tramite la tua configurazione di liz.
...
5. Puoi anche personalizzarli tramite la configurazione di liz. Ecco un esempio di specifica di un provider
...
Per usare le [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) con Liz:
...
Puoi anche personalizzare i modelli tramite la tua configurazione di liz. Ecco un esempio di come specificare l'ordine di routing dei provider.
...
:::tip
Puoi usare qualsiasi provider OpenAI-compatible con liz. La maggior parte dei provider AI moderni offre API OpenAI-compatible.
:::
...
4. Crea o aggiorna il tuo file `liz.json` nella directory del progetto:
...
I campi `limit` permettono a LIZ AI BRASIL di capire quanto contesto rimane. I provider standard recuperano questi dati da models.dev automaticamente.
...
1. **Controlla il setup auth**: Esegui `liz auth list` per vedere se le credenziali
...
2. Per provider personalizzati, controlla la config di liz e:
   - Assicurati che l'ID provider usato nel comando `/connect` corrisponda all'ID nella tua config liz.
