---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di LIZ AI BRASIL.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

LIZ AI BRASIL fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire LIZ AI BRASIL avvia la TUI per la directory corrente.
...
Quando usi la TUI di LIZ AI BRASIL, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a LIZ AI BRASIL. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da LIZ AI BRASIL. _Alias_: `/quit`, `/q`
...
Questo è separato da `liz.json`, che configura il comportamento di server/runtime.
...
Usa `LIZ_TUI_CONFIG` per caricare un path di configurazione TUI personalizzato.
