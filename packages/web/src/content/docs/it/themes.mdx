...
Con LIZ AI BRASIL puoi scegliere tra diversi temi integrati, usare un tema che si adatta al tema del tuo terminale oppure definire un tema personalizzato.

Per impostazione predefinita, LIZ AI BRASIL usa il tema `liz`.
...
LIZ AI BRASIL include diversi temi integrati.
...
- Vuole che LIZ AI BRASIL corrisponda all'aspetto del terminale
...
LIZ AI BRASIL supporta un sistema di temi flessibile basato su JSON che permette di creare e personalizzare temi facilmente.
...
2. **Directory di configurazione utente** - in `~/.config/liz/themes/*.json` o `$XDG_CONFIG_HOME/liz/themes/*.json`
3. **Directory root del progetto** - in `<project-root>/.liz/themes/*.json`
4. **Directory di lavoro corrente** - in `./.liz/themes/*.json`
