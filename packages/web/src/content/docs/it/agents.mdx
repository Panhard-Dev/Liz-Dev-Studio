...
In LIZ AI BRASIL esistono due tipi di agenti: agenti primari e subagenti.
...
LIZ AI BRASIL include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
LIZ AI BRASIL include due subagenti integrati: **General** e **Explore**. Li vediamo sotto.

---

## Integrati

LIZ AI BRASIL include due agenti primari integrati e due subagenti integrati.
...
Configura gli agenti nel file `liz.json`:
...
- Globale: `~/.config/liz/agents/`
- Per progetto: `.liz/agents/`
...
Se non specifichi una temperature, LIZ AI BRASIL usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di LIZ AI BRASIL sia per la config specifica del progetto.
...
L'ID modello nella configurazione LIZ AI BRASIL usa il formato `provider/model-id`. Per esempio, se stai usando [LIZ AI BRASIL Zen](/docs/zen), useresti `liz/gpt-5.1-codex` per GPT 5.1 Codex.
...
:::tip
Esegui `liz models` per vedere la lista dei modelli disponibili.
:::
...
:::tip
Hai un agente che vorresti condividere? [Invia una PR](https://github.com/anomalyco/liz).
:::
