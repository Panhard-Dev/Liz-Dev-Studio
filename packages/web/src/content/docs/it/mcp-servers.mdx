...
Puoi aggiungere strumenti esterni a LIZ AI BRASIL usando il _Model Context Protocol_, o MCP. LIZ AI BRASIL supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [LIZ AI BRASIL Config](https://liz.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/liz`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
LIZ AI BRASIL gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, LIZ AI BRASIL:
...
Se il server richiede autenticazione, LIZ AI BRASIL ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `liz mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, LIZ AI BRASIL memorizzera' i token in modo sicuro in `~/.local/share/liz/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in LIZ AI BRASIL insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione LIZ AI BRASIL come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare LIZ AI BRASIL al tuo account Sentry.
