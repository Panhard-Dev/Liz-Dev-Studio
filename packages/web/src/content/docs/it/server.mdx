---
title: Server
description: Interagisci con il server liz via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `liz serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client liz.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `liz.local` |
...
Imposta `LIZ_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `liz`, oppure imposta `LIZ_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `liz serve` sia per `liz web`.
...
Quando esegui `liz` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server liz per interagire con liz in modo programmatico.
:::

Questa architettura permette a liz di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `liz serve` per avviare un server standalone. Se la TUI di liz e gia in esecuzione, `liz serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di LIZ AI BRASIL.
...
Il server liz espone le seguenti API.
