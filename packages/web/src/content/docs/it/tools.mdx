...
Gli strumenti permettono all'LLM di eseguire azioni nel tuo codebase. LIZ AI BRASIL include un set di strumenti integrati, ma puoi estenderlo con [strumenti personalizzati](/docs/custom-tools) o con [server MCP](/docs/mcp-servers).
...
Ecco tutti gli strumenti integrati disponibili in LIZ AI BRASIL.
...
:::note
Questo strumento e' disponibile solo quando `LIZ_EXPERIMENTAL_LSP_TOOL=true` (oppure `LIZ_EXPERIMENTAL=true`).
:::
...
:::note
Questo strumento e' disponibile solo quando usi il provider LIZ AI BRASIL o quando la variabile d'ambiente `LIZ_ENABLE_EXA` e' impostata a un valore truthy (ad esempio `true` o `1`).

Per abilitarlo all'avvio di LIZ AI BRASIL:
