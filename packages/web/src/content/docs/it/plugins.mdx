---
title: Plugin
description: Scrivi plugin per estendere LIZ AI BRASIL.
---

I plugin ti permettono di estendere LIZ AI BRASIL agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di LIZ AI BRASIL.
...
- `.liz/plugins/` - plugin a livello progetto
- `~/.config/liz/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/liz/node_modules/`.
...
1. Config globale (`~/.config/liz/liz.json`)
2. Config di progetto (`liz.json`)
3. Directory plugin globale (`~/.config/liz/plugins/`)
4. Directory plugin di progetto (`.liz/plugins/`)
...
LIZ AI BRASIL esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di liz per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere liz.
...
:::note
Se usi l'app desktop di LIZ AI BRASIL, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a liz di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a liz:
...
L'helper `tool` crea uno strumento personalizzato che liz può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in liz insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://liz.ai/docs/sdk) per i dettagli.
