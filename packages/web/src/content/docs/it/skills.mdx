...
Le skill degli agenti permettono a LIZ AI BRASIL di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
LIZ AI BRASIL cerca in queste posizioni:

- Config di progetto: `.liz/skills/<name>/SKILL.md`
- Config globale: `~/.config/liz/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, LIZ AI BRASIL risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.liz/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/liz/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.liz/skills/git-release/SKILL.md` cosi':
...
LIZ AI BRASIL elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `liz.json`:
...
**Per agenti integrati** (in `liz.json`):
