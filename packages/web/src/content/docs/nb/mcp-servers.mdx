...
Du kan legge til eksterne verktøy i LIZ AI BRASIL ved å bruke _Model Context Protocol_ eller MCP. LIZ AI BRASIL støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [LIZ AI BRASIL Config](https://liz.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="liz.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/liz`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
LIZ AI BRASIL håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil LIZ AI BRASIL:
...
Hvis serveren krever autentisering, vil LIZ AI BRASIL be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `liz mcp auth <server-name>`.
...
```bash
liz mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil LIZ AI BRASIL lagre tokens sikkert i `~/.local/share/liz/mcp-auth.json`.
...
MCP-ene dine er tilgjengelige som verktøy i LIZ AI BRASIL, sammen med innebygde verktøy. Så du kan administrere dem gjennom LIZ AI BRASIL-konfigurasjonen som alle andre verktøy.
...
Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble LIZ AI BRASIL til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver LIZ AI BRASIL-samtale oppført som en separat økt.
