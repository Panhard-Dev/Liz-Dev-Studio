...
description: Skriv dine egne programtillegg for å utvide LIZ AI BRASIL.
---

Plugins lar deg utvide LIZ AI BRASIL ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til LIZ AI BRASIL.
...
- `.liz/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/liz/node_modules/`.
...
1. Global konfigurasjon (`~/.config/liz/liz.json`)
2. Prosjektkonfigurasjon (`liz.json`)
3. Global plugin-katalog (`~/.config/liz/plugins/`)
4. Prosjektpluginkatalog (`.liz/plugins/`)
...
LIZ AI BRASIL kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En LIZ AI BRASIL SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide LIZ AI BRASIL.
...
:::note
Hvis du bruker LIZ AI BRASIL-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre LIZ AI BRASIL fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til LIZ AI BRASIL:
...
`tool`-hjelperen lager et tilpasset verktøy som LIZ AI BRASIL kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for LIZ AI BRASIL sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://liz.ai/docs/sdk) for detaljer.
