---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i LIZ AI BRASIL.
---

import config from "../../../../config.mjs"
export const console = config.console

LIZ AI BRASIL bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i LIZ AI BRASIL-konfigurasjonen.
...
i `~/.local/share/liz/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i LIZ AI BRASIL
konfig.
...
LIZ AI BRASIL Zen er en liste over modeller levert av LIZ AI BRASIL-teamet som har vært
testet og verifisert for å fungere godt med LIZ AI BRASIL. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med LIZ AI BRASIL Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `LIZ AI BRASIL Zen` og gå til [liz.ai/auth](https://liz.ai/zen).
...
Det fungerer som alle andre leverandører i LIZ AI BRASIL og er helt valgfritt å bruke.

---

## LIZ AI BRASIL Go

LIZ AI BRASIL Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av LIZ AI BRASIL-teamet som har vært
testet og verifisert for å fungere godt med LIZ AI BRASIL.

1. Kjør kommandoen `/connect` i TUI, velg `LIZ AI BRASIL Go`, og gå til [liz.ai/auth](https://liz.ai/zen).
...
Slik bruker du Amazon Bedrock med LIZ AI BRASIL:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører LIZ AI BRASIL:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `liz.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i liz støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere liz til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at LIZ AI BRASIL skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom LIZ AI BRASIL-konfigurasjonen.
...
2. Velg autentiseringsmetoden din:
   ...
   3. Navn: `LIZ AI BRASIL`, omfang: `api`
...
for å lagre token i LIZ AI BRASIL auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
LIZ AI BRASIL bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse LIZ AI BRASIL
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`liz.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [liz-gitlab-auth](https://www.npmjs.com/package/liz-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `liz.json`:
...
Slik bruker du GitHub Copilot-abonnementet med LIZ AI BRASIL:
...
Slik bruker du Google Vertex AI med LIZ AI BRASIL:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører LIZ AI BRASIL.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom LIZ AI BRASIL, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [liz-helicone-session](https://github.com/H2Shami/liz-helicone-session) plugin for automatisk å logge hver LIZ AI BRASIL-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver LIZ AI BRASIL-samtale oppført som en separat økt.
...
Du kan konfigurere LIZ AI BRASIL for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere LIZ AI BRASIL for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere LIZ AI BRASIL for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for LIZ AI BRASIL. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/liz) for detaljer.
:::
...
Slik bruker du Ollama Cloud med LIZ AI BRASIL:
...
3. Kopier API-nøkkelen for bruk i LIZ AI BRASIL.
...
6. **Viktig**: Før du bruker skymodeller i LIZ AI BRASIL, må du hente modellinformasjonen lokalt:
...
LIZ AI BRASIL Zen er en liste over testede og verifiserte modeller levert av LIZ AI BRASIL-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>LIZ AI BRASIL Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **LIZ AI BRASIL Zen**.

   ```txt
   /connect
   ```

3. Skriv inn LIZ AI BRASIL API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom LIZ AI BRASIL-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom LIZ AI BRASIL-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med LIZ AI BRASIL:
...
Du kan også tilpasse modeller gjennom LIZ AI BRASIL-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med LIZ AI BRASIL. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `liz.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar LIZ AI BRASIL forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `liz auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk LIZ AI BRASIL-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i LIZ AI BRASIL-konfigurasjonen.
