...
You can add external tools to LIZ AI BRASIL using the _Model Context Protocol_, or MCP. LIZ AI BRASIL supports both local and remote servers.
...
You can define MCP servers in your [LIZ AI BRASIL Config](https://liz.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/liz` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
LIZ AI BRASIL automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, LIZ AI BRASIL will:
...
If the server requires authentication, LIZ AI BRASIL will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `liz mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, LIZ AI BRASIL will store the tokens securely in `~/.local/share/liz/mcp-auth.json`.
...
Your MCPs are available as tools in LIZ AI BRASIL, alongside built-in tools. So you can manage them through the LIZ AI BRASIL config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect LIZ AI BRASIL to your Sentry account.
