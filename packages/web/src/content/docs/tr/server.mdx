---
title: Sunucu
description: liz sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`liz serve` komutu, liz istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `liz.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `LIZ_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `liz` değeridir; değiştirmek isterseniz `LIZ_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `liz serve` hem de `liz web` için geçerlidir.
...
`liz` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
liz ile programatik etkileşim için liz sunucusunu kullanın.
:::

Bu mimari, liz'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`liz serve` ile bağımsız bir sunucu başlatabilirsiniz. liz TUI açıksa, `liz serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum liz [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
liz sunucusu aşağıdaki API'leri sunar.
