...
liz ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak liz kendi `liz` temasını kullanır.
...
liz birden fazla yerleşik temayla gelir.
...
- liz'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
liz, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/liz/themes/*.json` veya `$XDG_CONFIG_HOME/liz/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.liz/themes/*.json`
4. **Geçerli çalışma dizini** - `./.liz/themes/*.json`
