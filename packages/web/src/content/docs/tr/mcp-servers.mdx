...
_Model Bağlam Protokolü_ veya MCP kullanarak liz'a harici araçlar ekleyebilirsiniz. liz hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [liz Config](https://liz.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/liz` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
liz, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde liz şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa liz, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `liz mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından liz, belirteçleri `~/.local/share/liz/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra liz'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi liz yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve liz'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
