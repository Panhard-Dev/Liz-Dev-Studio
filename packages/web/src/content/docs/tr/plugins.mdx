---
title: Eklentiler
description: LIZ AI BRASIL'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek LIZ AI BRASIL'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya LIZ AI BRASIL'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.liz/plugins/` - Proje düzeyinde eklentiler
- `~/.config/liz/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/liz/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/liz/liz.json`)
2. Project config (`liz.json`)
3. Global eklenti dizini (`~/.config/liz/plugins/`)
4. Proje eklenti dizini (`.liz/plugins/`)
...
LIZ AI BRASIL bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir LIZ AI BRASIL SDK istemcisi.
...
LIZ AI BRASIL'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
LIZ AI BRASIL masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
LIZ AI BRASIL'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca LIZ AI BRASIL'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, LIZ AI BRASIL'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra LIZ AI BRASIL'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://liz.ai/docs/sdk) bakın.
