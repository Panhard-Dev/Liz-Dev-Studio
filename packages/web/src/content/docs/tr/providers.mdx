---
title: Sağlayıcılar
description: liz'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

liz, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı liz yapılandırmanızda yapılandırın.
...
`~/.local/share/liz/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları liz'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
LIZ AI BRASIL Zen, liz ekibi tarafından sağlanan modellerin bir listesidir.
liz ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz LIZ AI BRASIL Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, liz'u seçin ve [liz.ai/auth](https://liz.ai/auth)'ye gidin.
...
liz'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## LIZ AI BRASIL Go

LIZ AI BRASIL Go, liz ile iyi çalıştığı test edilmiş ve doğrulanmış, liz ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `LIZ AI BRASIL Go`'yu seçin ve [liz.ai/auth](https://liz.ai/zen) adresine gidin.
...
Amazon Bedrock'u liz ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   liz'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `liz.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi liz'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
liz'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://atomic.chat) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   liz'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
5. Bir model seçmek için `/models` komutunu çalıştırın.
   ...
   Liz yapılandırmanız aracılığıyla da modeller ekleyebilirsiniz.
...
2. Kimlik doğrulama yönteminizi seçin:
   ...
   **OAuth**'u seçtiğinizde tarayıcınız yetkilendirme için açılacaktır.
   ...
   3. Name: `LIZ AI BRASIL`, Scopes: `api`
...
to store token in liz auth storage.
:::

##### Kendi Kendine Barındırılan GitLab

:::note[uyumluluk notu]
liz, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. liz'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`liz.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
Daha fazla belge [liz-gitlab-auth](https://www.npmjs.com/package/liz-gitlab-auth) ana sayfasında.

##### Yapılandırma

`liz.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi liz'la kullanmak için:
...
Google Vertex AI'yi liz ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları liz çalıştırılırken ayarlayın.
...
Helicone'un liz aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her liz görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [liz-helicone-session](https://github.com/H2Shami/liz-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her liz görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
liz'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla liz'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla liz'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini liz için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/liz)'a bakın.
:::
...
Ollama Cloud'u liz ile kullanmak için:
...
3. liz'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: liz'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
LIZ AI BRASIL Zen, liz ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>LIZ AI BRASIL Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **LIZ AI BRASIL Zen**'i arayın.

   ```txt
   /connect
   ```

3. liz API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   liz yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca liz yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
[Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/)'yi Liz ile kullanmak için:
...
Modelleri liz yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı liz ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `liz.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları liz'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `liz auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için liz yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, liz yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
