...
Ajan becerileri, liz'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
liz su konumlari tarar:

- Proje konfigurasyonu: `.liz/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/liz/skills/<name>/SKILL.md`
...
Proje yerel yollarinda liz, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.liz/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/liz/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.liz/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
liz, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`liz.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`liz.json` icinde):
