...
LIZ AI BRASIL Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

:::note
LIZ AI BRASIL Go şu anda beta olarak mevcut.
:::

Go, LIZ AI BRASIL'daki diğer sağlayıcılar gibi çalışır. LIZ AI BRASIL Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve LIZ AI BRASIL'u kullanmak için bunu kullanmanıza gerek yoktur.
...
LIZ AI BRASIL Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

LIZ AI BRASIL Go, LIZ AI BRASIL'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>LIZ AI BRASIL Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `LIZ AI BRASIL Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye LIZ AI BRASIL Go'ya abone olabilir.
:::
...
LIZ AI BRASIL Go aşağıdaki limitleri içerir:
...
LIZ AI BRASIL yapılandırmanızdaki [model id](/docs/config/#models) formatı `liz-go/<model-id>` şeklindedir. Örneğin, Kimi K2.6 için yapılandırmanızda `liz-go/kimi-k2.6` kullanmalısınız.
...
LIZ AI BRASIL Go'yu şu amaçlarla oluşturduk:
...
4. LIZ AI BRASIL ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
