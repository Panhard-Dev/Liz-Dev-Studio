...
Para depurar problemas con LIZ AI BRASIL, comience verificando los registros y los datos locales que almacena en el disco.
...
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\liz\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `liz --log-level DEBUG`.

---

## Almacenamiento

liz almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/liz/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\liz`
...
LIZ AI BRASIL Desktop ejecuta un servidor LIZ AI BRASIL local (el sidecar `liz-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `LIZ AI BRASIL` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/liz/liz.jsonc` (o `~/.config/liz/liz.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/liz/liz.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\liz\liz.jsonc`
...
LIZ AI BRASIL también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:

- **Complementos globales**
  - **macOS/Linux**: `~/.config/liz/plugins/`
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\liz\plugins`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que LIZ AI BRASIL pueda reconstruirlo.

1. Salga de LIZ AI BRASIL Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/liz`
- **Linux**: eliminar `~/.cache/liz` (o ejecutar `rm -rf ~/.cache/liz`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\liz`

3. Reinicie el escritorio LIZ AI BRASIL.

---

### Solucionar problemas de conexión del servidor

LIZ AI BRASIL Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `liz.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.

#### Verificar variables de entorno

Si tiene `LIZ_PORT` configurado en su entorno, la aplicación de escritorio intentará usar ese puerto para el servidor local.

- Desarme `LIZ_PORT` (o elija un puerto libre) y reinicie.
...
En Windows, el escritorio LIZ AI BRASIL requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de LIZ AI BRASIL.

---

### Notificaciones que no se muestran

LIZ AI BRASIL Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para LIZ AI BRASIL en la configuración de su sistema operativo, y
...
1. Salga del escritorio LIZ AI BRASIL.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio LIZ AI BRASIL):

- `liz.settings.dat` (URL del servidor predeterminado de escritorio)
- `liz.global.dat` y `liz.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
Si tiene problemas con LIZ AI BRASIL:
...
3. Asegúrese de tener la última versión con `liz upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `liz models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\liz`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. liz instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\liz`

2. Reinicie liz para reinstalar los paquetes del proveedor más recientes.

Esto obligará a liz a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
liz detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
