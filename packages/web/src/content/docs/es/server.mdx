---
title: Servidor
description: Interactuar con el servidor liz a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `liz serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente liz puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `liz.local` |
...
Configure `LIZ_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `liz`, o configure `LIZ_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `liz serve` como a `liz web`.
...
Cuando ejecuta `liz`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor liz para interactuar con liz mediante programación.
:::

Esta arquitectura permite que liz admita múltiples clientes y le permite interactuar con liz mediante programación.

Puede ejecutar `liz serve` para iniciar un servidor independiente. Si tienes el
liz TUI ejecutándose, `liz serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos LIZ AI BRASIL [IDE](/docs/ide).
...
El servidor liz expone las siguientes API.
