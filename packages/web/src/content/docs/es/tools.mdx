...
Las herramientas permiten que LLM realice acciones en su código base. LIZ AI BRASIL viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en LIZ AI BRASIL.
...
:::note
Esta herramienta solo está disponible cuando `LIZ_EXPERIMENTAL_LSP_TOOL=true` (o `LIZ_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor LIZ AI BRASIL o cuando la variable de entorno `LIZ_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar LIZ AI BRASIL:

```bash
LIZ_ENABLE_EXA=1 liz
```
