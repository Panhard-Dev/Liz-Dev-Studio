...
Puede agregar herramientas externas a LIZ AI BRASIL utilizando el _Model Context Protocol_ o MCP. LIZ AI BRASIL admite servidores locales y remotos.
...
Puede definir servidores MCP en su [LIZ AI BRASIL Config](https://liz.ai/docs/config/) en `mcp`. Agregue cada MCP con un nombre único. Puede referirse a ese MCP por su nombre cuando solicite el LLM.
...
Las organizaciones pueden proporcionar servidores MCP predeterminados a través de su punto final `.well-known/liz`. Estos servidores pueden estar deshabilitados de forma predeterminada, lo que permite a los usuarios optar por los que necesitan.
...
LIZ AI BRASIL maneja automáticamente la autenticación OAuth para servidores MCP remotos. Cuando un servidor requiere autenticación, LIZ AI BRASIL:
...
Si el servidor requiere autenticación, LIZ AI BRASIL le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `liz mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, LIZ AI BRASIL almacenará los tokens de forma segura en `~/.local/share/liz/mcp-auth.json`.
...
Sus MCP están disponibles como herramientas en LIZ AI BRASIL, junto con herramientas integradas. Para que puedas administrarlos a través de la configuración LIZ AI BRASIL como cualquier otra herramienta.
...
Esto abrirá una ventana del navegador para completar el flujo OAuth y conectar LIZ AI BRASIL a su cuenta Sentry.
