---
title: Complementos
description: Escriba sus propios complementos para extender LIZ AI BRASIL.
---

Los complementos le permiten extender LIZ AI BRASIL al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de LIZ AI BRASIL.
...
- `.liz/plugins/` - Complementos a nivel de proyecto
- `~/.config/liz/plugins/` - Complementos globales
...
```json title="liz.json"
{
  ...
  "plugin": ["liz-helicone-session", "liz-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/liz/node_modules/`.
...
1. Configuración global (`~/.config/liz/liz.json`)
2. Configuración del proyecto (`liz.json`)
3. Directorio global de complementos (`~/.config/liz/plugins/`)
4. Directorio de complementos del proyecto (`.liz/plugins/`)
...
LIZ AI BRASIL ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".liz/plugins/my-plugin.ts"
...
```js title=".liz/plugins/example.js"
...
- `client`: Un cliente SDK liz para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar liz.
...
```js title=".liz/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "liz"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio LIZ AI BRASIL, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que liz lea archivos `.env`:

```javascript title=".liz/plugins/env-protection.js"
...
```javascript title=".liz/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a liz:

```ts title=".liz/plugins/custom-tools.ts"
import { type Plugin, tool } from "@liz-ai-brasil/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que liz puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para liz junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://liz.ai/docs/sdk) para obtener más detalles.
...
```ts title=".liz/plugins/compaction.ts"
import type { Plugin } from "@liz-ai-brasil/plugin"
...
```ts title=".liz/plugins/custom-compaction.ts"
import type { Plugin } from "@liz-ai-brasil/plugin"
