...
description: Cree herramientas que LLM pueda llamar en liz.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de liz como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.liz/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/liz/tools/`.
...
```ts title=".liz/tools/database.ts" {1}
...
```ts title=".liz/tools/project.ts" {8}
import { tool } from "@liz-ai-brasil/plugin"
...
```ts title=".liz/tools/python-add.ts" {10}
import { tool } from "@liz-ai-brasil/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".liz/tools/add.py")
