---
title: Providers
description: Using any LLM provider in LIZ AI BRASIL.
---

import config from "../../../config.mjs"
export const console = config.console

LIZ AI BRASIL uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your LIZ AI BRASIL config.
...
You can customize the providers through the `provider` section in your LIZ AI BRASIL
config.
...
LIZ AI BRASIL Zen is a list of models provided by the LIZ AI BRASIL team that have been
tested and verified to work well with LIZ AI BRASIL. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with LIZ AI BRASIL Zen.
:::

1. Run the `/connect` command in the TUI, select `LIZ AI BRASIL Zen`, and head to [liz.ai/auth](https://liz.ai/zen).
...
It works like any other provider in LIZ AI BRASIL and is completely optional to use.

---

## LIZ AI BRASIL Go

LIZ AI BRASIL Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the LIZ AI BRASIL team that have been
tested and verified to work well with LIZ AI BRASIL.

1. Run the `/connect` command in the TUI, select `LIZ AI BRASIL Go`, and head to [liz.ai/auth](https://liz.ai/zen).
...
To use Amazon Bedrock with LIZ AI BRASIL:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   Set one of these environment variables while running liz:
   ...
   ***
   ...
   For project-specific or persistent configuration, use `liz.json`:
...
LIZ AI BRASIL. Anthropic explicitly prohibits this.

Previous versions of LIZ AI BRASIL came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in LIZ AI BRASIL with zero setup:
...
You can configure liz to use local models through [Atomic Chat](https://atomic.chat), a desktop application that runs local LLMs behind an OpenAI-compatible API server (default endpoint `http://127.0.0.1:1337/v1`).
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for liz to work properly.
   :::
...
6. Run the `/models` command to select a model.
   ...
   You can also add models through your liz config.
...
GitLab Duo support in LIZ AI BRASIL is experimental. Features, configuration, and
behavior may change in future releases.
:::

LIZ AI BRASIL integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
LIZ AI BRASIL uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock LIZ AI BRASIL
to only use your own GitLab-hosted instance, add the following to your
`liz.json` file. It is also recommended to disable session sharing.
...
More documentation on [liz-gitlab-auth](https://www.npmjs.com/package/liz-gitlab-auth) homepage.

##### Configuration

Customize through `liz.json`:
...
When a `duo-workflow-*` model is selected, LIZ AI BRASIL will:
...
4. Route tool execution requests through LIZ AI BRASIL's permission-gated tool system
...
To use your GitHub Copilot subscription with liz:
...
To use Google Vertex AI with LIZ AI BRASIL:
...
In the event you see a feature or model from Helicone that isn't configured automatically through liz, you can always configure it yourself.
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [liz-helicone-session](https://github.com/H2Shami/liz-helicone-session) plugin to automatically log each LIZ AI BRASIL conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each LIZ AI BRASIL conversation listed as a separate session.
...
You can configure liz to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility
...
You can configure liz to use local models through LM Studio.
...
You can configure liz to use local models through Ollama.

:::tip
Ollama can automatically configure itself for LIZ AI BRASIL. See the [Ollama integration docs](https://docs.ollama.com/integrations/liz) for details.
:::
...
To use Ollama Cloud with LIZ AI BRASIL:
...
3. Copy the API key for use in LIZ AI BRASIL.
...
6. **Important**: Before using cloud models in LIZ AI BRASIL, you must pull the model information locally:
...
LIZ AI BRASIL Zen is a list of tested and verified models provided by the LIZ AI BRASIL team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>LIZ AI BRASIL Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **LIZ AI BRASIL Zen**.

   ```txt
   /connect
   ```

3. Enter your LIZ AI BRASIL API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your liz config.
...
5. You can also customize them through your liz config. Here's an example of specifying a provider
...
To use [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) with Liz:
...
You can also customize models through your liz config. Here's an example of specifying provider routing order.
...
:::tip
You can use any OpenAI-compatible provider with liz. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
4. Create or update your `liz.json` file in your project directory:
...
The `limit` fields allow LIZ AI BRASIL to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `liz auth list` to see if the credentials
...
2. For custom providers, check the liz config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your liz config.
