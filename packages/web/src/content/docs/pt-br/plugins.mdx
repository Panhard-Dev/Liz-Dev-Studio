---
title: Plugins
description: Escreva seus próprios plugins para estender o liz.
---

Plugins permitem que você estenda o liz conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do liz.
...
- `.liz/plugins/` - Plugins em nível de projeto
- `~/.config/liz/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/liz/node_modules/`.
...
1. Configuração global (`~/.config/liz/liz.json`)
2. Configuração do projeto (`liz.json`)
3. Diretório de plugins global (`~/.config/liz/plugins/`)
4. Diretório de plugins do projeto (`.liz/plugins/`)
...
O liz executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do liz para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o liz.
...
:::note
Se você estiver usando o aplicativo desktop liz, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o liz de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao liz:
...
O helper `tool` cria uma ferramenta personalizada que o liz pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o liz junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://liz.ai/docs/sdk) para detalhes.
