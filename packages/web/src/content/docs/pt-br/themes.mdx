...
Com o liz, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o liz usa nosso próprio tema `liz`.
...
O liz vem com vários temas embutidos.
...
- Querem que o liz corresponda à aparência do seu terminal
...
O liz suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/liz/themes/*.json` ou `$XDG_CONFIG_HOME/liz/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.liz/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.liz/themes/*.json`
