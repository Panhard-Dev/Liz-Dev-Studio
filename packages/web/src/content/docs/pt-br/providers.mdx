---
title: Provedores
description: Usando qualquer provedor LLM no liz.
---

import config from "../../../../config.mjs"
export const console = config.console

liz usa o [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) para suportar **75+ provedores LLM** e suporta a execução de modelos locais.

Para adicionar um provedor, você precisa:

1. Adicionar as chaves da API para o provedor usando o comando `/connect`.
2. Configurar o provedor na sua configuração do liz.

---

### Credenciais

Quando você adiciona as chaves da API de um provedor com o comando `/connect`, elas são armazenadas em `~/.local/share/liz/auth.json`.

---

### Configuração

Você pode personalizar os provedores através da seção `provider` na sua configuração do liz.
...
LIZ AI BRASIL Zen é uma lista de modelos fornecidos pela equipe do liz que foram testados e verificados para funcionar bem com o liz. [Saiba mais](/docs/zen).

:::tip
Se você é novo, recomendamos começar com o LIZ AI BRASIL Zen.
:::

1. Execute o comando `/connect` no TUI, selecione `LIZ AI BRASIL Zen` e acesse [liz.ai/zen](https://liz.ai/zen).
...
Funciona como qualquer outro provedor no liz e é completamente opcional.

---

## LIZ AI BRASIL Go

LIZ AI BRASIL Go é um plano de assinatura de baixo custo que fornece acesso confiável a modelos de codificação abertos populares fornecidos pela equipe do liz que foram
testados e verificados para funcionar bem com o liz.

1. Execute o comando `/connect` no TUI, selecione `LIZ AI BRASIL Go` e acesse [liz.ai/zen](https://liz.ai/zen).
...
Para usar o Amazon Bedrock com o liz:
...
2. **Configure a autenticação** usando um dos seguintes métodos:
   ...
   ***
   ...
   Defina uma dessas variáveis de ambiente ao executar o liz:
   ...
   ***
   ...
   Para configuração específica do projeto ou persistente, use `liz.json`:
...
:::info
Usar sua assinatura Claude Pro/Max no liz não é oficialmente suportado pela [Anthropic](https://anthropic.com).
:::
...
Você pode configurar o liz para usar modelos locais através do [Atomic Chat](https://atomic.chat), um aplicativo de desktop que executa LLMs locais por trás de um servidor API compatível com OpenAI (endpoint padrão `http://127.0.0.1:1337/v1`).
...
2. Vá para [Azure AI Foundry](https://ai.azure.com/) e implante um modelo.
   ...
   :::note
   O nome da implantação deve corresponder ao nome do modelo para que o liz funcione corretamente.
   :::
...
5. Execute o comando `/models` para selecionar um modelo.
   ...
   Você também pode adicionar modelos através da sua configuração do liz.
...
2. Escolha seu método de autenticação:
   ...
   3. Nome: `LIZ AI BRASIL`, Escopos: `api`
...
armazenar o token no armazenamento de autenticação do liz.
:::

##### GitLab Auto-Hospedado

:::note[nota de conformidade]
liz usa um pequeno modelo para algumas tarefas de AI, como gerar o título da sessão.
Ele está configurado para usar gpt-5-nano por padrão, hospedado pela Zen. Para bloquear o liz
para usar apenas sua própria instância hospedada do GitLab, adicione o seguinte ao seu
arquivo `liz.json`. Também é recomendado desativar o compartilhamento de sessões.
...
Mais documentação na página [liz-gitlab-auth](https://www.npmjs.com/package/liz-gitlab-auth).

##### Configuração

Personalize através do `liz.json`:
...
Para usar sua assinatura do GitHub Copilot com o liz:
...
Para usar o Google Vertex AI com o liz:
...
Caso você veja um recurso ou modelo do Helicone que não está configurado automaticamente através do liz, você sempre pode configurá-lo você mesmo.
...
O recurso [Sessões](https://docs.helicone.ai/features/sessions) do Helicone permite que você agrupe solicitações LLM relacionadas. Use o plugin [liz-helicone-session](https://github.com/H2Shami/liz-helicone-session) para registrar automaticamente cada conversa do liz como uma sessão no Helicone.
...
O plugin injeta os cabeçalhos `Helicone-Session-Id` e `Helicone-Session-Name` em suas solicitações. Na página de Sessões do Helicone, você verá cada conversa do liz listada como uma sessão separada.
...
Você pode configurar o liz para usar modelos locais através da utilidade llama-server do [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Você pode configurar o liz para usar modelos locais através do LM Studio.
...
Você pode configurar o liz para usar modelos locais através do Ollama.

:::tip
Ollama pode se configurar automaticamente para o liz. Consulte a [documentação de integração do Ollama](https://docs.ollama.com/integrations/liz) para detalhes.
:::
...
Para usar o Ollama Cloud com o liz:
...
3. Copie a chave da API para uso no liz.
...
6. **Importante**: Antes de usar modelos em nuvem no liz, você deve puxar as informações do modelo localmente:
...
LIZ AI BRASIL Zen é uma lista de modelos testados e verificados fornecidos pela equipe do liz. [Saiba mais](/docs/zen).

1. Faça login em **<a href={console}>LIZ AI BRASIL Zen</a>** e clique em **Criar Chave da API**.

2. Execute o comando `/connect` e procure por **LIZ AI BRASIL Zen**.

   ```txt
   /connect
   ```

3. Insira sua chave da API do liz.
...
4. Muitos modelos do OpenRouter são pré-carregados por padrão, execute o comando `/models` para selecionar o que você deseja.
   ...
   Você também pode adicionar modelos adicionais através da sua configuração do liz.
...
5. Você também pode personalizá-los através da sua configuração do liz. Aqui está um exemplo de especificar um provedor
...
Para usar [APIs Generativas Scaleway](https://www.scaleway.com/en/docs/generative-apis/) com o liz:
...
Você também pode personalizar modelos através da sua configuração do liz. Aqui está um exemplo de especificar a ordem de roteamento do provedor.
...
:::tip
Você pode usar qualquer provedor compatível com OpenAI com o liz. A maioria dos provedores de AI modernos oferece APIs compatíveis com OpenAI.
:::
...
3. Insira sua chave da API para o provedor.
   ...
   ┌  Adicionar credencial
   │
   ▲  Isso armazena apenas uma credencial para myprovider - você precisará configurá-lo no liz.json, verifique a documentação para exemplos.
   │
...
4. Crie ou atualize seu arquivo `liz.json` no diretório do seu projeto:
...
Os campos `limit` permitem que o liz entenda quanto contexto você tem disponível. Provedores padrão puxam isso automaticamente do models.dev.
...
1. **Verifique a configuração de autenticação**: Execute `liz auth list` para ver se as credenciais
...
2. Para provedores personalizados, verifique a configuração do liz e:
   - Certifique-se de que o ID do provedor usado no comando `/connect` corresponda ao ID na sua configuração do liz.
