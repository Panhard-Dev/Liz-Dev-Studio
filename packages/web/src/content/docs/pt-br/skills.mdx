...
As habilidades do agente permitem que o liz descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O liz pesquisa nesses locais:

- Configuração do projeto: `.liz/skills/<name>/SKILL.md`
- Configuração global: `~/.config/liz/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o liz sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.liz/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/liz/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.liz/skills/git-release/SKILL.md` assim:
...
O liz lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `liz.json`:
...
**Para agentes embutidos** (em `liz.json`):
