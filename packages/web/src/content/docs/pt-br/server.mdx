---
title: Servidor
description: Interaja com o servidor liz via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `liz serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente liz pode usar.
...
```bash
liz serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `liz.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
liz serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `LIZ_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `liz`, ou defina `LIZ_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `liz serve` quanto ao `liz web`.

```bash
LIZ_SERVER_PASSWORD=your-password liz serve
```
...
Quando você executa `liz`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor liz para interagir com o liz programaticamente.
:::

Essa arquitetura permite que o liz suporte múltiplos clientes e permite que você interaja com o liz programaticamente.

Você pode executar `liz serve` para iniciar um servidor autônomo. Se você tiver o TUI do liz em execução, `liz serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do liz [IDE](/docs/ide).
...
O servidor liz expõe as seguintes APIs.
