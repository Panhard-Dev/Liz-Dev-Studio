...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@liz/acme")).toBe("@liz/acme")
    expect(Npm.sanitize("@liz/acme@1.0.0")).toBe("@liz/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/liz/acme.git"
    const expected = win ? "acme@git+https_//github.com/liz/acme.git" : spec
