...
import { List } from "@liz-ai-brasil/ui/list"
import type { ListRef } from "@liz-ai-brasil/ui/list"
import { getDirectory, getFilename } from "@liz-ai-brasil/core/util/path"
...
function useDirectorySearch(args: {
  ...
  const roots = createMemo(() => {
    const homeRoot = rootOf(args.home())
    const startRoot = rootOf(args.start() ?? "")
    const preferred = [homeRoot, startRoot, "C:/", "D:/", "E:/", "/"]
      .map((item) => normalizeDriveRoot(item))
      .filter((item) => !!item && !!rootOf(item))
      .map((item) => trimTrailing(item))

    return Array.from(new Set(preferred))
  })
  const cache = new Map<string, Promise<Array<{ name: string; absolute: string }>>>()
  const rootCache = new Map<string, Promise<boolean>>()
  let current = 0

  const scoped = (value: string) => {
    const raw = normalizeDriveRoot(value)
    const base = args.start() || roots()[0]
  ...
  const rootExists = async (root: string) => {
    const key = trimTrailing(root)
