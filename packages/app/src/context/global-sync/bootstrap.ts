import type {
  ...
  Session,
  Todo,
} from "@liz-ai-brasil/sdk/v2/client"
...
export const loadGlobalConfigQuery = (
  sdk?: LizClient,
  transform?: (x: Awaited<ReturnType<LizClient["global"]["config"]["get"]>>) => void,
) =>
  queryOptions({
    queryKey: ["config"],
    queryFn: sdk
...
export const loadProjectsQuery = (
  sdk?: LizClient,
  transform?: (x: Awaited<ReturnType<LizClient["project"]["list"]>>["data"]) => void,
) =>
  queryOptions({
    queryKey: ["project"],
    queryFn: sdk
          ...
          retry(() =>
            sdk.project
              ...
              .then((x) => {
                return (x.data ?? [])
                  .filter((p) => !!p?.id)
                  .filter((p) => !!p.worktree && !p.worktree.includes("liz-test"))
                  .slice()
...
export const loadProvidersQuery = (directory: string | null, sdk?: LizClient) =>
  queryOptions({
...
export const loadAgentsQuery = (
  ...
  sdk?: LizClient,
  transform?: (x: Awaited<ReturnType<LizClient["app"]["agents"]>>) => void,
) =>
  queryOptions({
...
export const loadPathQuery = (
  directory: string | null,
  sdk?: LizClient,
  transform?: (x: Awaited<ReturnType<LizClient["path"]["get"]>>) => void,
) =>
  queryOptions<Path>({
