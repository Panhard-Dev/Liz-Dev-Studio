...
import { type Session } from "@liz-ai-brasil/sdk/v2/client"
import {
  childSessionOnPath,
  displayName,
...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("liz://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("liz://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("liz://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("liz://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("liz://open-project")).toBeUndefined()
    expect(parseDeepLink("liz://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "liz://open-project?directory=/a",
      "liz://other?directory=/b",
      "liz://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("liz://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("liz://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("liz://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("liz://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "liz://new-session?directory=/a",
      "liz://open-project?directory=/b",
      "liz://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __LIZ__: {
        deepLinks: ["liz://open-project?directory=/a"],
      },
    } as unknown as Window & { __LIZ__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["liz://open-project?directory=/a"])
