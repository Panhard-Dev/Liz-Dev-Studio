...
import { base64Encode } from "@liz-ai-brasil/core/util/encode"
import { decode64 } from "@/utils/base64"
import { ResizeHandle } from "@liz-ai-brasil/ui/resize-handle"
import { Button } from "@liz-ai-brasil/ui/button"
import { IconButton } from "@liz-ai-brasil/ui/icon-button"
import { Tooltip } from "@liz-ai-brasil/ui/tooltip"
import { DropdownMenu } from "@liz-ai-brasil/ui/dropdown-menu"
import { Dialog } from "@liz-ai-brasil/ui/dialog"
import { getFilename } from "@liz-ai-brasil/core/util/path"
import { Session, type Message } from "@liz-ai-brasil/sdk/v2/client"
...
import { showToast, Toast, toaster } from "@liz-ai-brasil/ui/toast"
...
import { Binary } from "@liz-ai-brasil/core/util/binary"
import { retry } from "@liz-ai-brasil/core/util/retry"
...
import { useTheme, type ColorScheme } from "@liz-ai-brasil/ui/theme/context"
...
const DISCONNECTED_LIZ_ACCOUNT: LizAccount = { connected: false }
...
export default function Layout(props: ParentProps) {
  ...
  const [lizAccount, { refetch: refetchLizAccount }] = createResource(
    () => globalSDK.url,
    async () => {
      const response = await globalSDK.fetch("/liz/account", {
        headers: {
          Accept: "application/json",
        },
      })
      const data = (await response.json().catch(() => undefined)) as LizAccount | undefined
      return data ?? DISCONNECTED_LIZ_ACCOUNT
    },
    { initialValue: DISCONNECTED_LIZ_ACCOUNT },
  )
  const lizAccountValue = createMemo(() => lizAccount.latest ?? DISCONNECTED_LIZ_ACCOUNT)
  const lizProfileLabel = createMemo(() => {
    const account = lizAccountValue()
    if (!account.connected) return "Entrar na LIZ AI BRASIL"
    return account.email || account.accountId || "Conta LIZ conectada"
  })
  const lizProfileFallback = createMemo(() => {
    const account = lizAccountValue()
    if (!account.connected) return "Liz"
    return account.displayName || account.email || account.username || "Liz"
  })
  ...
  createEffect(() => {
    if (lizAccount.loading) return
    if (lizAccountValue().connected) return
    if (!providers.connected().some((provider) => provider.id === "liz")) return
    void globalSDK.client.global.dispose().catch(() => undefined)
  })
  ...
  const disconnectLizProfile = async () => {
    await globalSDK.client.auth.remove({ providerID: "liz" })
  ...
  const sidebarContent = (mobile?: boolean) => (
    <SidebarContent
      ...
      profileLabel={() => lizProfileLabel()}
      profileFallback={() => lizProfileFallback()}
      profileAvatarUrl={() => lizAccountValue().avatarUrl}
      profileConnected={() => lizAccountValue().connected}
      onOpenProfile={openLizProfile}
      onDisconnectProfile={disconnectLizProfile}
      settingsLabel={() => language.t("sidebar.settings")}
      settingsKeybind={() => command.keybind("settings.open")}
      onOpenSettings={openSettings}
      helpLabel={() => language.t("sidebar.help")}
      onOpenHelp={() => platform.openLink("https://liz.ai/desktop-feedback")}
      renderPanel={() =>
