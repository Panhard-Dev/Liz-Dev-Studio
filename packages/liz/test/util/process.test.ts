...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.LIZ_TEST ?? "")'), {
      env: {
        LIZ_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "LIZ_TEST_SHELL"], {
      shell: true,
      env: {
        LIZ_TEST_SHELL: "ok",
      },
    ...
    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("LIZ_TEST_SHELL=ok")
  })
