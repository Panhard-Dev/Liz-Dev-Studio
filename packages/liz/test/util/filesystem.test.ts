...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "liz.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "liz.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "liz.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "liz.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["liz.json", "liz.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "liz.json"),
        path.join(tmp.path, "liz.jsonc"),
        path.join(project, "liz.json"),
        path.join(project, "liz.jsonc"),
      ])
    })
  })
