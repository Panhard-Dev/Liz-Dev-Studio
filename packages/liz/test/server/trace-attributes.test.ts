...
describe("paramToAttributeKey", () => {
  ...
  test("namespaces non-ID params under liz.", () => {
    expect(paramToAttributeKey("name")).toBe("liz.name")
    expect(paramToAttributeKey("slug")).toBe("liz.slug")
  })
})
...
describe("requestAttributes", () => {
  ...
  test("emits OTel-style <domain>.id for ID-shaped route params", () => {
    ...
    expect(attrs["liz.sessionID"]).toBeUndefined()
    expect(attrs["liz.messageID"]).toBeUndefined()
    expect(attrs["liz.partID"]).toBeUndefined()
  })
  ...
  test("namespaces non-ID params under liz. (e.g. mcp :name)", () => {
    const attrs = requestAttributes(
      fakeContext("POST", "http://localhost/mcp/exa/connect", {
        name: "exa",
      }),
    )
    expect(attrs["liz.name"]).toBe("exa")
