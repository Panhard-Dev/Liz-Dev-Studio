...
const originalHttpApi = Flag.LIZ_EXPERIMENTAL_HTTPAPI
const it = testEffect(
...
function request(path: string, directory: string, init: RequestInit = {}) {
  return Effect.promise(() => {
    ...
    headers.set("x-liz-directory", directory)
...
afterEach(async () => {
  ...
  Flag.LIZ_EXPERIMENTAL_HTTPAPI = originalHttpApi
      ...
      try {
        const response = yield* request(url.toString(), dir, {
          ...
          headers: {
            "accept-encoding": "br",
            "content-type": "application/json",
            "x-liz-workspace": "internal",
          },
          body: JSON.stringify({ $schema: "https://liz.ai/config.json" }),
        })
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-liz-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-liz-workspace")
      } finally {
