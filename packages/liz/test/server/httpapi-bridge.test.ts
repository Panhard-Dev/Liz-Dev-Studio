...
function app(input?: { password?: string; username?: string }) {
  ...
  Flag.LIZ_SERVER_PASSWORD = input?.password
  Flag.LIZ_SERVER_USERNAME = input?.username
...
describe("HttpApi server", () => {
  test("keeps Effect HttpApi behind the feature flag", () => {
    Flag.LIZ_EXPERIMENTAL_HTTPAPI = false
  ...
  test("requires credentials when auth is enabled", async () => {
    ...
    const [missing, bad, good] = await Promise.all([
      app({ password: "secret" }).request(fileUrl(), {
        headers: { "x-liz-directory": tmp.path },
      }),
      app({ password: "secret" }).request(fileUrl(), {
        headers: {
          authorization: authorization("liz", "wrong"),
          "x-liz-directory": tmp.path,
        },
      }),
      app({ password: "secret" }).request(fileUrl(), {
        headers: {
          authorization: authorization("liz", "secret"),
          "x-liz-directory": tmp.path,
        },
      }),
    ])
  ...
  test("accepts auth_token query credentials", async () => {
    ...
    const response = await app({ password: "secret" }).request(
      fileUrl({ token: Buffer.from("liz:secret").toString("base64") }),
      {
        headers: {
          "x-liz-directory": tmp.path,
        },
  ...
  test("serves global health from Effect HttpApi", async () => {
    const response = await app().request(`${GlobalPaths.health}?directory=/does/not/exist/liz-test`)
