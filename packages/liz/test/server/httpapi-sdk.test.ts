...
import { createLizClient } from "@liz-ai-brasil/sdk/v2"
...
type Sdk = ReturnType<typeof createLizClient>
...
function app(backend: Backend, input?: { password?: string; username?: string }) {
  Flag.LIZ_EXPERIMENTAL_HTTPAPI = backend === "httpapi"
      ...
      Effect.gen(function* () {
        ...
        const bad = yield* capture(() =>
          client(backend, directory, {
            password: "secret",
            headers: { authorization: authorization("liz", "wrong") },
          }).file.read({ path: "hello.txt" }),
        )
        const good = yield* capture(() =>
          client(backend, directory, {
            password: "secret",
            headers: { authorization: authorization("liz", "secret") },
          }).file.read({ path: "hello.txt" }),
        )
