...
import { EffectFlock } from "@liz-ai-brasil/core/util/effect-flock"

import { Instance } from "../../src/project/instance"
import { Auth } from "../../src/auth"
import { Account } from "../../src/account/account"
import { AccessToken, AccountID, OrgID } from "../../src/account/schema"
import { AppFileSystem } from "@liz-ai-brasil/core/filesystem"
import { Env } from "../../src/env"
import { provideTmpdirInstance } from "../fixture/fixture"
import { tmpdir } from "../fixture/fixture"
import { CrossSpawnSpawner } from "@liz-ai-brasil/core/cross-spawn-spawner"
...
const managedConfigDir = process.env.LIZ_TEST_MANAGED_CONFIG_DIR!
...
async function writeManagedSettings(settings: object, filename = "liz.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "liz.json") {
...
test("updates global config and omits empty shell key in json", async () => {
  ...
  try {
    ...
    const writtenConfig = await Filesystem.readJson<{ shell?: string }>(path.join(tmp.path, "liz.json"))
...
test("updates global config and omits empty shell key in jsonc", async () => {
  ...
  try {
    ...
    const file = path.join(tmp.path, "liz.jsonc")
...
test("ignores legacy tui keys in liz config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://liz.ai/config.json",
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "liz.json"))
...
test("resolves env templates in account config with account token", async () => {
  const originalControlToken = process.env["LIZ_CONSOLE_TOKEN"]
  ...
  const fakeAccount = Layer.mock(Account.Service)({
    ...
    config: () =>
      Effect.succeed(
        Option.some({
          provider: { liz: { options: { apiKey: "{env:LIZ_CONSOLE_TOKEN}" } } },
        }),
      ),
    ...
    await provideTmpdirInstance(() =>
      Config.Service.use((svc) =>
        Effect.gen(function* () {
          ...
          expect(config.provider?.["liz"]?.options?.apiKey).toBe("st_test_token")
        }),
      ),
    ).pipe(Effect.scoped, Effect.provide(layer), Effect.provide(Npm.defaultLayer), Effect.runPromise)
  } finally {
    if (originalControlToken !== undefined) {
      process.env["LIZ_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["LIZ_CONSOLE_TOKEN"]
    }
  }
})
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "liz.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .liz directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const lizDir = path.join(dir, ".liz")
      await fs.mkdir(lizDir, { recursive: true })
      const agentDir = path.join(lizDir, "agent")
...
test("agent markdown permission config preserves user key order", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const agentDir = path.join(dir, ".liz", "agent")
      await fs.mkdir(agentDir, { recursive: true })
...
test("loads agents from .liz/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const lizDir = path.join(dir, ".liz")
      await fs.mkdir(lizDir, { recursive: true })

      const agentsDir = path.join(lizDir, "agents")
...
test("loads commands from .liz/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const lizDir = path.join(dir, ".liz")
      await fs.mkdir(lizDir, { recursive: true })

      const commandDir = path.join(lizDir, "command")
...
test("loads commands from .liz/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const lizDir = path.join(dir, ".liz")
      await fs.mkdir(lizDir, { recursive: true })

      const commandsDir = path.join(lizDir, "commands")
...
test("does not try to install dependencies in read-only LIZ_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.LIZ_CONFIG_DIR
  process.env.LIZ_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.LIZ_CONFIG_DIR
    else process.env.LIZ_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable LIZ_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(dir, "liz.json"),
        JSON.stringify({ $schema: "https://liz.ai/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .liz config
      const projectDir = path.join(dir, "project")
      const lizDir = path.join(projectDir, ".liz")
      await fs.mkdir(lizDir, { recursive: true })
      ...
      // Local .liz config with different plugins
      await Filesystem.write(
        path.join(lizDir, "liz.json"),
        JSON.stringify({
          $schema: "https://liz.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .liz config with some overlapping plugins
      await Filesystem.write(
        path.join(lizDir, "liz.json"),
        JSON.stringify({
          $schema: "https://liz.ai/config.json",
...
test("keeps plugin origins aligned with merged plugin list", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const local = path.join(project, ".liz")
...
// Note: preload.ts sets LIZ_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .liz config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .liz directory config enables it
      const lizDir = path.join(dir, ".liz")
      await fs.mkdir(lizDir, { recursive: true })
      await Filesystem.write(
        path.join(lizDir, "liz.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  globalThis.fetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/liz")) {
      fetchedUrl = urlStr
    ...
    await provideTmpdirInstance(
      () =>
        Config.Service.use((svc) =>
          Effect.gen(function* () {
            const config = yield* svc.get()
            expect(fetchedUrl).toBe("https://example.com/.well-known/liz")
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    await using tmp = await tmpdir()
    const file = path.join(tmp.path, "liz.json")
    expect(await ConfigPlugin.resolvePluginSpec("oh-my-liz@2.4.3", file)).toBe("oh-my-liz@2.4.3")
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-liz@2.4.3", "file:///project/.liz/plugin/oh-my-liz.js"]
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = ["file:///project/.liz/plugin/demo.ts", "file:///project/.liz/plugin/demo.ts"]

    const result = dedupe(plugins)

    expect(result).toEqual(["file:///project/.liz/plugin/demo.ts"])
  })
  ...
  test("loads auto-discovered local plugins as file urls", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const lizDir = path.join(projectDir, ".liz")
        const pluginDir = path.join(lizDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("LIZ_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["LIZ_DISABLE_PROJECT_CONFIG"]
    process.env["LIZ_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["LIZ_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["LIZ_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .liz/ directories when flag is set", async () => {
    const originalEnv = process.env["LIZ_DISABLE_PROJECT_CONFIG"]
    process.env["LIZ_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .liz directory with a command
          const lizDir = path.join(dir, ".liz", "command")
          await fs.mkdir(lizDir, { recursive: true })
          await Filesystem.write(path.join(lizDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          // Project .liz should NOT be in directories list
          const hasProjectLiz = directories.some((d) => d.startsWith(tmp.path))
          expect(hasProjectLiz).toBe(false)
        },
      })
    } finally {
      if (originalEnv === undefined) {
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["LIZ_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["LIZ_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["LIZ_CONFIG_DIR"]
      process.env["LIZ_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["LIZ_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["LIZ_CONFIG_DIR"]
      } else {
        process.env["LIZ_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("LIZ_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["LIZ_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["LIZ_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["LIZ_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["LIZ_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await load()
          // Should load from LIZ_CONFIG_DIR, not project
...
describe("LIZ_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in LIZ_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["LIZ_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["LIZ_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://liz.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["LIZ_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["LIZ_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in LIZ_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["LIZ_CONFIG_CONTENT"]
  ...
  const config = ConfigParse.effectSchema(
    ...
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(
        JSON.stringify({
          PayloadDisplayName: "LIZ AI BRASIL Managed",
          PayloadIdentifier: "ai.liz.managed.test",
          PayloadType: "ai.liz.managed",
...
test("parseManagedPlist handles empty config", async () => {
  const config = ConfigParse.effectSchema(
    Config.Info,
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(JSON.stringify({ $schema: "https://liz.ai/config.json" })),
      "test:mobileconfig",
    ),
    "test:mobileconfig",
  )
  expect(config.$schema).toBe("https://liz.ai/config.json")
})
