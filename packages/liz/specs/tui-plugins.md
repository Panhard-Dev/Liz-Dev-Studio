...
- Author package entrypoint is `@liz-ai-brasil/plugin/tui`.
...
- npm packages can be TUI theme-only via `package.json["liz-themes"]` without a `./tui` entrypoint.
...
- Import types from `@liz-ai-brasil/plugin/tui`.
- `@liz-ai-brasil/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@liz-ai-brasil/plugin/tui"
...
- If a configured TUI package has no `./tui` entrypoint and no valid `liz-themes`, it is skipped with a warning (not a load failure).
- If a configured TUI package has no `./tui` entrypoint but has valid `liz-themes`, runtime creates a no-op module record and still loads it for theme sync and plugin state.
...
- `tui` target when `liz-themes` exists and resolves to a non-empty set of valid package-relative theme paths.

`liz-themes` rules:

- `liz-themes` is an array of relative paths.
- Absolute paths and `file://` paths are rejected.
- Resolved theme paths must stay inside the package directory.
- Invalid `liz-themes` causes manifest read failure for install.

Example:

```json
{
  "name": "@acme/liz-plugin",
  "type": "module",
  "main": "./dist/server.js",
  "exports": {
    "./server": {
...
- The value is a semver range checked against the running LIZ AI BRASIL version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.liz` is absent, no check is performed (backward compatible).
...
- `liz plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `liz plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.liz` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.liz`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.liz`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.liz` dir or `LIZ_CONFIG_DIR`, LIZ AI BRASIL installs `@liz-ai-brasil/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@liz-ai-brasil/plugin/tui`.
...
- `api.theme.install(...)` and `liz-themes` auto-sync share the same installer path.
...
- Local plugins persist installed themes under the local `.liz/themes` area near the plugin config source.
...
- `api.plugins.add(spec)` can load theme-only packages (`liz-themes` with no `./tui`) as runtime entries.
...
- `--pure` / `LIZ_PURE` skips external TUI plugins only.
- External plugin resolution and import are parallel.
- Packages with no `./tui` entrypoint and valid `liz-themes` are loaded as synthetic no-op TUI plugin modules.
- Theme-only packages loaded this way appear in `api.plugins.list()` and plugin manager rows like other external plugins.
- Packages with no `./tui` entrypoint and no valid `liz-themes` are skipped with warning.
- External plugin activation is sequential to keep command, route, and side-effect order deterministic.
- Theme auto-sync from `liz-themes` runs before plugin `tui(...)` execution and only on metadata state `first` or `updated`.
...
- `tui` target detection includes `exports["./tui"]` and valid `liz-themes`.
...
- Local smoke plugin: `.liz/plugins/tui-smoke.tsx`
- Local vim plugin: `.liz/plugins/tui-vim.tsx`
- Local smoke config: `.liz/tui.json`
- Local smoke theme: `.liz/plugins/smoke-theme.json`
