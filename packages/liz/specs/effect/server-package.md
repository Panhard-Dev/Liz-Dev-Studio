...
Practical reference for extracting a future `packages/server` from the current `packages/liz` monolith while `packages/core` is still being migrated to Effect.
...
- `packages/core` - all liz services, Effect-first source of truth
- `packages/server` - liz server, with separate contract and implementation, still producing `openapi.json`
...
- import from `server` and embed the full liz server into an existing Node app
- spawn the CLI and talk to the server through that boundary

## Current state

Everything still lives in `packages/liz`.
...
- those slices are mounted into the Hono server behind `LIZ_EXPERIMENTAL_HTTPAPI`
...
The intended direction here is the same, but the current `liz` package split is earlier in the migration.
...
- the current server host still lives in `packages/liz`
...
- if `packages/server` imports services from `packages/liz` while `packages/liz` imports `packages/server` to host routes, we create a package cycle immediately
...
4. keep `packages/liz` as the temporary Hono host
...
- `packages/server` must not import from `packages/liz`

Allowed in phase 1:

- `packages/liz` imports `packages/server`
...
- `packages/liz` shrinks or disappears as package responsibilities are fully split
...
- `packages/liz` keeps generating legacy OpenAPI from Hono routes
...
- expose host adapters needed by `packages/liz`
...
- host mounting from `packages/liz`
...
- no imports from `packages/liz` inside `packages/server`
...
- `packages/server` must still not import from `packages/liz`
...
### PR 4. Mount `packages/server` question from `packages/liz`

Scope:

- replace local experimental question route wiring in `packages/liz`
...
- mount it in parallel from `packages/liz`
...
- move adapter selection and server startup out of `packages/liz`

Rules:

- do not start this while `packages/server` still depends on `packages/liz`
...
- host logic no longer lives in `packages/liz`
...
3. the route is mounted from the current host in `packages/liz`
...
- `packages/server` imports services or runtime from `packages/liz`
- `packages/liz` imports route definitions or handlers from `packages/server`

Avoid by:

- keeping phase-1 `packages/server` free of `packages/liz` imports
...
- [ ] mount `question` from `packages/liz`
...
2. keep `packages/liz` as the temporary host
...
If a proposed PR would make `packages/server` import from `packages/liz`, stop and restructure the boundary first.
