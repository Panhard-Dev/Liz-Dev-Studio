...
const MANAGED_PLIST_DOMAIN = "ai.liz.managed"

// Keys injected by macOS/MDM into the managed plist that are not LIZ AI BRASIL config
...
function systemManagedConfigDir(): string {
  switch (process.platform) {
    case "darwin":
      return "/Library/Application Support/liz"
    case "win32":
      return path.join(process.env.ProgramData || "C:\\ProgramData", "liz")
    default:
      return "/etc/liz"
  }
}
...
export function managedConfigDir() {
  return process.env.LIZ_TEST_MANAGED_CONFIG_DIR || systemManagedConfigDir()
}
