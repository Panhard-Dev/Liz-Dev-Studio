...
import { InstallationLocal, InstallationVersion } from "@liz-ai-brasil/core/installation/version"
...
function normalizeLoadedConfig(data: unknown, source: string) {
  ...
  delete copy.tui
  log.warn("tui keys in liz config are deprecated; move them to tui.json", { path: source })
  return copy
}
...
export const Info = Schema.Struct({
  ...
  server: Schema.optional(ConfigServer.Server).annotate({
    description: "Server configuration for liz serve and web commands",
  }),
  command: Schema.optional(Schema.Record(Schema.String, ConfigCommand.Info)).annotate({
    description: "Command configuration, see https://liz.ai/docs/commands",
  }),
  ...
  ).annotate({ description: "Agent configuration, see https://liz.ai/docs/agents" }),
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (!data.$schema) {
        data.$schema = "https://liz.ai/config.json"
    ...
    const loadGlobal = Effect.fnUntraced(function* () {
      ...
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "liz.json")))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "liz.jsonc")))
        ...
        yield* Effect.promise(() =>
          import(pathToFileURL(legacy).href, { with: { type: "toml" } })
            .then(async (mod) => {
              ...
              result["$schema"] = "https://liz.ai/config.json"
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "LIZ_CONFIG_CONTENT") return "local"
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            log.debug("fetching remote config", { url: `${url}/.well-known/liz` })
            const response = yield* Effect.promise(() => fetch(`${url}/.well-known/liz`))
            if (!response.ok) {
              throw new Error(`failed to fetch remote config from ${url}: ${response.status}`)
            }
            const wellknown = (yield* Effect.promise(() => response.json())) as { config?: Record<string, unknown> }
            const remoteConfig = wellknown.config ?? {}
            if (!remoteConfig.$schema) remoteConfig.$schema = "https://liz.ai/config.json"
            const source = `${url}/.well-known/liz`
        ...
        if (Flag.LIZ_CONFIG) {
          yield* merge(Flag.LIZ_CONFIG, yield* loadFile(Flag.LIZ_CONFIG))
          log.debug("loaded custom config", { path: Flag.LIZ_CONFIG })
        }

        if (!Flag.LIZ_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("liz", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        if (Flag.LIZ_CONFIG_DIR) {
          log.debug("loading config from LIZ_CONFIG_DIR", { path: Flag.LIZ_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void, never>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".liz") || dir === Flag.LIZ_CONFIG_DIR) {
            for (const file of ["liz.json", "liz.jsonc"]) {
          ...
          // Auto-discovered plugins under `.liz/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.LIZ_CONFIG_CONTENT) {
          const source = "LIZ_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.LIZ_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          log.debug("loaded custom config from LIZ_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["LIZ_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("LIZ_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.LIZ_PERMISSION) {
          result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.LIZ_PERMISSION))
        }
        ...
        if (Flag.LIZ_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.LIZ_DISABLE_PRUNE) {
