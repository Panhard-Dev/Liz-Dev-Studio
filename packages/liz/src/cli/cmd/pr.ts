...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run liz",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.code === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for liz session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found liz session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await Process.text(["liz", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        UI.println()
        UI.println("Starting liz...")
        UI.println()

        const lizArgs = sessionId ? ["-s", sessionId] : []
        const lizProcess = Process.spawn(["liz", ...lizArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
        })
        const code = await lizProcess.exited
        if (code !== 0) throw new Error(`liz exited with code ${code}`)
      },
    })
  },
})
