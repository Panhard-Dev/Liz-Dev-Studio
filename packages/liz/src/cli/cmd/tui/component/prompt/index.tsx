...
import type { AssistantMessage, FilePart, UserMessage } from "@liz-ai-brasil/sdk/v2"
...
export function Prompt(props: PromptProps) {
  ...
  const placeholderText = createMemo(() => {
    ...
    if (store.mode === "shell") {
      ...
      return `Rode um comando... "${example}"`
    }
    if (!list().length) return undefined
    return `Pergunte para a LIZ... "${list()[store.placeholder % list().length]}"`
  })
              ...
              <Switch>
                <Match when={store.mode === "normal"}>
                  <Switch>
                    ...
                    <Match when={true}>
                      <text fg={theme.text}>
                        {keybind.print("agent_cycle")} <span style={{ fg: theme.textMuted }}>agentes</span>
                      </text>
                    </Match>
                  </Switch>
                  <text fg={theme.text}>
                    {keybind.print("command_list")} <span style={{ fg: theme.textMuted }}>comandos</span>
                  </text>
                </Match>
                <Match when={store.mode === "shell"}>
                  <text fg={theme.text}>
                    esc <span style={{ fg: theme.textMuted }}>sair do shell</span>
                  </text>
                </Match>
              </Switch>
            </box>
