...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".liz" ? path.join(source_dir, "themes") : path.join(source_dir, ".liz", "themes")
...
async function readThemeFiles(spec: string, pkg?: PluginPackage) {
  ...
  return Promise.resolve()
    ...
    .catch((error) => {
      warn("invalid tui plugin liz-themes", {
        path: spec,
        pkg: pkg.pkg,
        error,
      })
...
async function syncPluginThemes(plugin: PluginEntry) {
  ...
  for (const file of plugin.load.theme_files) {
    await install(file).catch((error) => {
      warn("failed to sync tui plugin liz-themes", { path: plugin.load.spec, id: plugin.id, theme: file, error })
    })
  }
}
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".liz", "tui.json"),
  }
}
...
async function installPluginBySpec(
  ...
  if (!manifest.ok) {
    if (manifest.code === "manifest_no_targets") {
      return {
        ok: false,
        message: `"${spec}" does not expose plugin entrypoints or liz-themes in package.json`,
      }
    }
...
async function load(input: { api: Api; config: TuiConfig.Info }) {
  ...
  try {
    await Instance.provide({
      directory: cwd,
      fn: async () => {
        const records = Flag.LIZ_PURE ? [] : (config.plugin_origins ?? [])
        if (Flag.LIZ_PURE && config.plugin_origins?.length) {
