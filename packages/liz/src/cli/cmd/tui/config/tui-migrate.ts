...
const TUI_SCHEMA_URL = "https://liz.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from liz.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const liz = await lizFiles(input)
  for (const file of liz) {
...
async function lizFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "liz"),
    ...(await Filesystem.findUp(["liz.json", "liz.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "liz"))
  }
  if (Flag.LIZ_CONFIG) files.push(Flag.LIZ_CONFIG)
