...
import liz from "./theme/liz.json" with { type: "json" }
...
import type { TuiThemeCurrent } from "@liz-ai-brasil/plugin/tui"
  ...
  init: (props: { mode: "dark" | "light" }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "liz")
        draft.active = typeof active === "string" ? active : "liz"
        draft.ready = false
      }),
    )
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.liz, store.mode)
    })
