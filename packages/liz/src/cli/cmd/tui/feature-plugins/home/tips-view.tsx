...
const themeTip = `Use {highlight}/themes{/highlight} ou {highlight}Ctrl+X T{/highlight} para alternar entre ${themeCount} temas da LIZ`
...
const NO_MODELS_TIP = "Rode {highlight}/connect{/highlight} para conectar um provedor de IA e programar com a LIZ"
...
const TIPS = [
  "Digite {highlight}@{/highlight} seguido de um arquivo para buscar e anexar contexto",
  "Start a message with {highlight}!{/highlight} to run shell commands directly (e.g., {highlight}!ls -la{/highlight})",
  "Press {highlight}Tab{/highlight} to cycle between Build and Plan agents",
  "Use {highlight}/undo{/highlight} to revert the last message and file changes",
  "Use {highlight}/redo{/highlight} to restore previously undone messages and file changes",
  "Run {highlight}/share{/highlight} to create a public link to your conversation at liz.ai",
  ...
  "Rode {highlight}/connect{/highlight} para adicionar chaves de API dos provedores de IA suportados",
  ...
  "Create {highlight}liz.json{/highlight} for server settings and {highlight}tui.json{/highlight} for TUI settings",
  "Place TUI settings in {highlight}~/.config/liz/tui.json{/highlight} for global config",
  "Add {highlight}$schema{/highlight} to your config for autocomplete in your editor",
  "Configure {highlight}model{/highlight} in config to set your default model",
  "Override any keybind in {highlight}tui.json{/highlight} via the {highlight}keybinds{/highlight} section",
  "Set any keybind to {highlight}none{/highlight} to disable it completely",
  "Configure local or remote MCP servers in the {highlight}mcp{/highlight} config section",
  "LIZ AI BRASIL auto-handles OAuth for remote MCP servers requiring auth",
  "Add {highlight}.md{/highlight} files to {highlight}.liz/command/{/highlight} to define reusable custom prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks in commands to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.liz/agent/{/highlight} for specialized AI personas",
  "Configure per-agent permissions for {highlight}edit{/highlight}, {highlight}bash{/highlight}, and {highlight}webfetch{/highlight} tools",
  'Use patterns like {highlight}"git *": "allow"{/highlight} for granular bash permissions',
  'Set {highlight}"rm -rf *": "deny"{/highlight} to block destructive commands',
  'Configure {highlight}"git push": "ask"{/highlight} to require approval before pushing',
  "LIZ AI BRASIL auto-formats files using prettier, gofmt, ruff, and more",
  'Set {highlight}"formatter": false{/highlight} in config to disable all auto-formatting',
  "Define custom formatter commands with file extensions in config",
  "LIZ AI BRASIL uses LSP servers for intelligent code analysis",
  "Create {highlight}.ts{/highlight} files in {highlight}.liz/tools/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.liz/plugin/{/highlight} for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent LIZ AI BRASIL from reading sensitive files",
  "Use {highlight}liz run{/highlight} for non-interactive scripting",
  "Use {highlight}liz --continue{/highlight} to resume the last session",
  "Use {highlight}liz run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}liz serve{/highlight} for headless API access to LIZ AI BRASIL",
  "Use {highlight}liz run --attach{/highlight} to connect to a running server",
  "Run {highlight}liz upgrade{/highlight} to update to the latest version",
  "Run {highlight}liz auth list{/highlight} to see all configured providers",
  "Run {highlight}liz agent create{/highlight} for guided agent creation",
  "Use {highlight}/liz{/highlight} in GitHub issues/PRs to trigger AI actions",
  "Run {highlight}liz github install{/highlight} to set up the GitHub workflow",
  "Comment {highlight}/liz fix this{/highlight} on issues to auto-create PRs",
  "Comment {highlight}/oc{/highlight} on PR code lines for targeted code reviews",
  'Use {highlight}"theme": "system"{/highlight} to match your terminal\'s colors',
  "Create JSON theme files in {highlight}.liz/themes/{/highlight} directory",
  ...
  "Run {highlight}liz debug config{/highlight} to troubleshoot configuration",
  ...
  "Run {highlight}docker run -it --rm ghcr.io/liz-ai-brasil/liz{/highlight} for containerized use",
  "Use {highlight}/models{/highlight} to see Liz 2.3, Liz 2.5 PRO and Liz 2.6 PRO",
