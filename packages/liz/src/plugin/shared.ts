...
export const DEPRECATED_PLUGIN_PACKAGES = ["liz-openai-codex-auth", "liz-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, lizVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(lizVersion) || semver.major(lizVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.liz
  if (typeof range !== "string") return
  if (!semver.satisfies(lizVersion, range)) {
    throw new Error(`Plugin requires liz ${range} but running ${lizVersion}`)
  }
}
...
export function readPackageThemes(spec: string, pkg: PluginPackage) {
  const field = pkg.json["liz-themes"]
  if (field === undefined) return []
  if (!Array.isArray(field)) {
    throw new TypeError(`Plugin ${spec} has invalid liz-themes field`)
  }

  const list = field.map((item) => {
    if (typeof item !== "string") {
      throw new TypeError(`Plugin ${spec} has invalid liz-themes entry`)
    }

    const raw = item.trim()
    if (!raw) {
      throw new TypeError(`Plugin ${spec} has empty liz-themes entry`)
    }
    if (raw.startsWith("file://") || isAbsolutePath(raw)) {
      throw new TypeError(`Plugin ${spec} liz-themes entry must be relative: ${item}`)
    }

    return resolvePackageFile(spec, raw, "liz-themes", pkg)
  })
