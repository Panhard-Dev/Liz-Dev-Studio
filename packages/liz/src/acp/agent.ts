...
import type { AssistantMessage, Event, LizClient, SessionMessageResponse, ToolPart } from "@liz-ai-brasil/sdk/v2"
...
export async function init({ sdk: _sdk }: { sdk: LizClient }) {
  return {
...
export class Agent implements ACPAgent {
  ...
  async initialize(params: InitializeRequest): Promise<InitializeResponse> {
    ...
    const authMethod: AuthMethod = {
      description: "Run `liz auth login` in the terminal",
      name: "Login with liz",
      id: "liz-login",
    }
  ...
  private async processMessage(message: SessionMessageResponse) {
    ...
    for (const part of message.parts) {
      if (part.type === "tool") {
        ...
        // LIZ AI BRASIL stores files internally as { type: "file", url, filename, mime }.
...
async function defaultModel(config: ACPConfig, cwd?: string): Promise<{ providerID: ProviderID; modelID: ModelID }> {
  ...
  const lizProvider = providers.find((p) => p.id === "liz")
  if (lizProvider) {
    const [best] = Provider.sort(Object.values(lizProvider.models))
    if (best) {
      return {
  ...
  return { providerID: ProviderID.liz, modelID: ModelID.make("liz-2.6-pro") }
}
