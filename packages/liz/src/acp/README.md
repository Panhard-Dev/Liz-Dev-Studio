...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for liz.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal liz sessions
...
```bash
LIZ_ENABLE_QUESTION_TOOL=1 liz acp
```
...
- **Terminal Integration**: Full terminal support via liz's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | liz acp
```
...
### Mapping to LIZ AI BRASIL

ACP sessions map cleanly to liz's internal session model:
