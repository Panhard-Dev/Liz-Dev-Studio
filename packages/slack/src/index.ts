...
import { createLiz, type ToolPart } from "@liz-ai-brasil/sdk"
...
console.log("🚀 Starting liz server...")
const liz = await createLiz({
  port: 0,
})
console.log("✅ Liz server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
void (async () => {
  const events = await liz.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new liz session...")
    const { client, server } = liz
    ...
    console.log("✅ Created liz session:", createResult.data.id)
  ...
  console.log("📝 Sending to liz:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 Liz response:", JSON.stringify(result, null, 2))
