...
pub fn init(log_dir: &Path) -> WorkerGuard {
    ...
    let filename = format!("liz-desktop_{timestamp}.log");
    ...
    let filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        if cfg!(debug_assertions) {
            EnvFilter::new("liz_lib=debug,liz_desktop=debug,sidecar=debug")
        } else {
            EnvFilter::new("liz_lib=info,liz_desktop=info,sidecar=info")
        }
    });
