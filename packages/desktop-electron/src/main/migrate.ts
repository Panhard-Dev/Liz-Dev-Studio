...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.liz.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `liz.settings.dat` is special: it maps to the `liz.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"liz.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // liz.settings.dat → the electron settings store ("liz.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "liz.global.dat").
  const storeName = filename === "liz.settings.dat" ? "liz.settings" : filename
