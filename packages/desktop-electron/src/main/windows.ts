...
function windowIcon() {
  ...
  const fallback = nativeImage.createFromPath(join(iconsDir(), "icon.png"))
  return fallback.isEmpty() ? iconPath() : fallback
}

function applyWindowIcon(win: BrowserWindow) {
  if (process.platform !== "win32") return
  const icon = windowIcon()
  if (typeof icon !== "string") win.setIcon(icon)
}
