...
**Only** add if it's specifically about the Desktop application or `liz web` view. **Do not** add for terminal, TUI, or general liz issues.
...
**Only** add if the issue mentions "zen" or "liz zen" or "liz black".

If the issue doesn't have "zen" or "liz black" in it then don't add zen label

#### core

Use for core server issues in `packages/liz/`, excluding `packages/liz/src/cli/cmd/tui/`.
...
TUI (`packages/liz/src/cli/cmd/tui/...`):
...
Core (`packages/liz/...`, excluding TUI subtree):

- thdxr for sqlite/snapshot/memory bugs and larger architectural core features
- jlongster for liz server + API feature work (tool currently remaps jlongster -> thdxr until assignable)
