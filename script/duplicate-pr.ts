...
import { createLiz } from "@liz-ai-brasil/sdk"
...
`)
  ...
  const liz = await createLiz({ port: 0 })
  ...
  try {
    ...
    const session = await liz.client.session.create()
    const result = await liz.client.session
      .prompt({
