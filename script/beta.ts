...
async function build() {
  ...
  try {
    await $`./script/build.ts --single`.cwd("packages/liz")
    return true
  } catch (err) {
...
async function fix(pr: PR, files: string[], prs: PR[], applied: number[], idx: number) {
  console.log(`  Trying to auto-resolve ${files.length} conflict(s) with liz...`)
  ...
  try {
    await $`liz run -m ${model} ${prompt}`
  } catch (err) {
    console.log(`  liz failed: ${err}`)
    return false
  }
  ...
  console.log("  Conflicts resolved with liz")
  return true
}
...
async function smoke(prs: PR[], applied: number[]) {
  ...
  console.log("\nTrying to fix final smoke check with liz...")
  ...
  const prompt = [
    ...
    "Run `./script/build.ts --single` in `packages/liz`.",
